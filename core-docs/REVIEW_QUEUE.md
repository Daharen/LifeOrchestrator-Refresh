...
## Ninth producer wired (Module 24)
`gen.music` is the **ninth skill that appends** to `review_queue.jsonl` (after `model.gateway`, `classify.batch`, `speech.stt`, `speech.tts`, `ocr.layout`, `detect.objects`, `image.interpret`, `gen.image`), and the second from the **generator** track. It generates one short instrumental clip from a text prompt with a local **MusicGen Small** model (transformers) and flags a **failed, silent, or low-level** generation. Its confidence is a documented **generation-completeness / non-silent heuristic** (not calibrated) from the audio RMS: silent/failed (rms <=0.005) -> 0.1, very-low-level (<0.02) -> 0.3, low-level (<0.05) -> 0.5, has-content (>=0.05) -> 0.9 (with a duration-shortfall guard capping a truncated clip). When the envelope `confidence` falls below `-ConfidenceThreshold` (default 0.5), it appends **one** `lifeorch.review.item/0.1` with `flagged_by:"gen.music"`, `source_ref:"artifact://<invDir>/gen.json"`, `weak_result = {model, prompt_preview, duration_s, rms, confidence_reason}`, `requested:"verify_generation"` (the same verb as `gen.image`), and a **reason**: `failed_transform` (silent, confidence <=0.15) or `low_confidence` (low-level). A stronger model / the frontier can judge whether the music actually matches the prompt (a completeness signal cannot). Its optional child (`audio.ingest` for a non-wav `-Format`/`-SampleRate`) is deterministic and writes nothing to the queue. Module 9 selects by `flagged_by` and handles `verify_generation` by construction. Verified by the Module 24 mock gate (silent -> `failed_transform`, low-rms -> `low_confidence`, a good clip -> no item) with the canonical queue verified untouched live (1->1, `m24-test-002`). **Producers are now 7/8/11/12/14/16/17/23/24 -> local drainer 9 -> frontier for the residue.** See D-0035.

## Tenth producer wired (Module 25)

`gen.video` is the **tenth skill that appends** to `review_queue.jsonl` (after `model.gateway`, `classify.batch`, `speech.stt`, `speech.tts`, `ocr.layout`, `detect.objects`, `image.interpret`, `gen.image`, `gen.music`), and the third from the **generator** track. It generates one short silent clip from a text prompt with a local **AnimateDiff-Lightning** pipeline (an SD 1.5 base + a 4-step motion adapter, diffusers) and flags a **failed, blank, or static** generation. Its confidence is a documented **generation-completeness / non-blank + non-static heuristic** (not calibrated) from the frame pixel std (blank/uniform <=2 -> 0.1, very-low-detail <8 -> 0.3, low-detail <15 -> 0.5, has-content >=15 -> 0.9) PLUS a **motion guard** on the mean inter-frame difference (a clip that does not move is a still image, not a video: interframe_diff <=0.5 -> 0.3 static_no_motion, <1.5 -> 0.5 low_motion). When the envelope `confidence` falls below `-ConfidenceThreshold` (default 0.5), it appends **one** `lifeorch.review.item/0.1` with `flagged_by:"gen.video"`, `source_ref:"artifact://<invDir>/gen.json"`, `weak_result = {model, prompt_preview, num_frames, pixel_std, interframe_diff, confidence_reason}`, `requested:"verify_generation"` (the same verb as `gen.image`/`gen.music`), and a **reason**: `failed_transform` (blank/failed, confidence <=0.15) or `low_confidence` (low-detail/static). A stronger model / the frontier can judge whether the clip actually matches the prompt and moves coherently (a completeness signal cannot). Module 9 selects by `flagged_by` and handles `verify_generation` by construction. Verified by the Module 25 mock gate (blank -> `failed_transform`, static -> `low_confidence`, a good clip -> no item) with the canonical queue verified untouched live (before==after, `m25-test-002`). **Producers are now 7/8/11/12/14/16/17/23/24/25 -> local drainer 9 -> frontier for the residue.** See D-0036.

## Non-producer: res.lease (Module 29)
`res.lease` (Module 29, the resource-arbitration lock/lease layer) is a **deterministic tool, NOT a review-queue producer** (like `doc.io` #20, `fs.manage` #28, `gen.audio` #22): it acquires/releases/renews named filesystem leases and decides nothing to review -- `confidence:null`, empty `model_provenance`, it never writes `review_queue.jsonl`. The **producer set stays at ten** (7/8/11/12/14/16/17/23/24/25). See D-0053.

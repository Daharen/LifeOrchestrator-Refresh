...
- **Lane:** -- (convention: GPU lane -- <=1 GPU per wave)
- **Worker id / label:** --
- **Module/area (exclusive):** --
- **GPU:** false (set true ONLY if this is the wave's single GPU worker)
- **Docs:** `[]`

## Mission
_EMPTY -- no unit assigned._ Fill this slot at wave scoping from the candidate menu in
`core-docs/FANOUT_ORCHESTRATOR_HANDOFF.md` section 4 (the i36 CONSUMER-GLUE / Tier-1-flip / P0-1 units). Do NOT copy
the menu here -- paste the `orchestrate.fanout` `plan`-emitted worker prompt (or a tight summary pointing to the
emitted copy + the governing design doc) into the Unit section, keep the slot within its 8 KB budget, mirror it.
...
- Report: `-Action report -State done` + a plain measured summary (negative results are first-class, the D-0061 ethos).

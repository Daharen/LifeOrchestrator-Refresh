...
| `model.gateway` (M7) | `modules/07-model-gateway/Invoke-ModelGateway.ps1 [-Model <id>\|-Tier <tiny\|weak\|mid\|strong>] -Prompt <s> [-System -MaxTokens -Temperature -Seed -GpuLayers -Context -LoadTimeoutSec -EvictWarm -EnsureResident -PoolStatus -SweepIdle -ClearSlot]` | **the single entry point to local text LLMs** (llama.cpp `llama-server`); mixed · **ps:false** (port + most of VRAM) · **prod**; no streaming | base 42/42 + warm 23/23 + pool 43/43 + off-machine 228/228 + **lease-split 63/63** + live split proof (i21, 0.5.0 `-UsePoolLeaseSplit`), 0 orphans, 2026-07-31 · D-0016/44/55/57/62/67/76 |
...
| `agent.local` (M21) | `modules/21-agent-local/Invoke-AgentLocal.ps1 -Goal <s> [-WorkingDir -MaxSteps <8> -DryRun -Route] [-Profile <frugal\|floor\|max>] [-AutoRamp[:$false]\|-NoAutoRamp] [-AllowLegacy27B] [-DecisionTiers -GenTier -Seed -MaxTokens -ToolsPath]` | bounded ReAct loop (#19 decide → #7 args → child skill → observe); **the registry IS the capability surface — NO arbitrary-shell tool**; **ps:false** · non-producer | 102/102 + AutoRamp 122/122 + **autoramp-split 22/22** (i21, autoramp 0.2.0 `-SplitLease`) + live floor-check PASS 2026-07-31 · D-0032/41/43/46/59–62 |
...
| `gen.image` (M23) | `modules/23-gen-image/Invoke-GenImage.ps1 -Prompt <text> [-NegativePrompt -Width -Height -Steps -Guidance -Seed] [-Scheduler <dpm++\|euler\|euler_a\|ddim>] [-Format <png\|jpg\|webp>] [-Model\|-Tier]` | SD 1.5 (default) + SD 3.5 Medium fp16 quality tier (`-Tier sd35`, i17/D-0070) text-to-image (diffusers, speech venv); **ps:false** (CUDA) · **prod**; ~2–3 s @512², **~2.6 GB VRAM**; **`safety_checker` DISABLED** | **32/32 `-Live`** `m23-test-005` 2026-07-25 · D-0034 |
...
| `res.lease` (M29) | `modules/29-resource-lease/Invoke-ResLease.ps1 <acquire\|release\|renew\|status\|list>` | filesystem lease/lock with TTL; atomic `CreateNew` reservation, race-safe stale reclaim, `lease_id`, **timezone-safe expiry**; det · **ps:true** | **74/74 + 36/36 v0.3 + 45/45 v0.4 `-Live`** (0.2.0 R1a fencing/split -> 0.3.0 R1b three-identity+atomic-transition -> 0.4.0 R1b' hardening: incarnation ids + two-phase transition-capability + target-fenced `fence-op` + idempotent saga journal + oplock renew + durable state_version; -> 0.4.1 (i21 command-evictor `tree_gone` passthrough); findings 1/13/14 CLOSE-ELIGIBLE at the lease/consumer layer via the i21 live proof, warm-pool default-ON gated on a supervisor-hardening wave, D-0076) `0877c70` 2026-07-31 · D-0072/76 |
...
**#7 pool manager Stage-1 (D-0067, i14, skill 0.3.0, `09a7e71`) — OPT-IN / default-OFF.** `-EnsureResident` adds `Ensure-ResidentModel(model_id, config_key)`: reuse ~1 ms on an exact expanded-residency-key match, else evict (confirm exit + VRAM recovery) + reload + `/health` + `/v1/models` provenance + a published manifest; plus `-PoolStatus` (read-only), `-SweepIdle` (90 s keep-resident), `-ClearSlot`. Whole-task `gpu` lease; native `--models` router / `--slot-save-path` DEFERRED Stage-2+. Frontier red-team found 4 Critical hardening gaps — **do NOT enable by default until Stage-1.1** (WARM_POOL_DESIGN §10).
...
| `image.sd35-medium` | image-gen | `23-gen-image\stable-diffusion-3.5-medium\` (diffusers fp16; 6 safetensors sha256-verified) | 15.16 GB | diffusers | via `gen.image` #23 (`-Tier sd35`, i17/D-0070); StableDiffusion3Pipeline fp16 + model CPU offload + VAE tiling, T5-XXL CPU. **torch VRAM peak ~12 GB — NOT a clean 11 GB fit** (T5 fp16 spike -> driver system-RAM fallback; sequential-offload ladder = guaranteed fallback). Stability Community License (free < $1M rev) |
...
- **Tiers (`models.json`):** LLM `tiny`=0.5B, `weak`=1.5B (gateway default), `mid`=3B (the decision floor),
  `strong`=**Qwen3.5-9B Q5_K_M**; detector `nano` (default) / `tiny`; vlm `3b`; image-gen `sd15` (+`sd35` quality tier); music-gen `small`.
...
- **Warm multi-model pool + router** — Stage-1 (mechanism C) SHIPPED opt-in/default-OFF (D-0067, i14); Stage-1.1 hardening backlog (WARM_POOL_DESIGN §10); native `--models` router still Stage-2+.

...
| `res.lease` (M29) | `modules/29-resource-lease/Invoke-ResLease.ps1 <acquire\|release\|renew\|status\|list>` | filesystem lease/lock with TTL; atomic `CreateNew` reservation, race-safe stale reclaim, `lease_id`, **timezone-safe expiry**; det · **ps:true** | **74/74 `-Live`** (v0.2.0 R1a: fencing_token + exec/residency_pin split + prepared/mock-evictor seam + lock-order) `e701328` 2026-07-30 · D-0072 |

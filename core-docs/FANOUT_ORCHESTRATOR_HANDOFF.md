...
- **i34 IN FLIGHT (D-0098): Tier-1 HIERARCHY -- PART 1 (contracts) COMMITTED `0bdb71d`, PART 2 DISPATCH-READY.**
  MEMORY_CONTRACT **A6** + CONTEXT_PACKET_CONTRACT **i34** fold the frontier DESIGN red-team (b4c90545 = NO-GO-as-drafted -- a bounded beam pruning on lossy synopses is NOT recall-preserving -- GO after the redraft): the SAFE-PRUNING invariant (navigation prioritizes, NEVER excludes a branch without a channel-specific no-false-negative proof) + generations/CAS staleness + authorization-bound frontier ops + write-time+transitive ns-homogeneity + hierarchy identity + minimal balanced bulk-build (live split -> i35); design+digest `research/2026-08-04-i34-hierarchy-design*.md`. PART 2 = the 3-lane CPU build (plan `fo-34-584fd656`, dispatch_now 3 / 0 gpu / 0 doc-contention / clean preflight): #36 0.4->0.5 (node layer + balanced bulk-builder + generations/CAS + homogeneity + authz shortlist/descend + safe-pruning predicates + schema 4->5; OWNS the gate tests), #37 (navigation-cost + DUAL recall [path + packet-evidence] + adversarial fixtures + the ~200MB rehearsal gate), #40 0.5->0.6 (shortlist-and-descend PLAN + safe-pruning enforcement + retrieval_completeness + navigation_refs). Slots 001/002/003 READY. **NEXT = Nicholas dispatches #36/#37/#40 -> poll `-Action status -PlanId fo-34-584fd656` -> `-Action handoff` -> the D-0077 HIERARCHY fold smoke (a real #36 tree -> #40 shortlist-and-descend compile: sub-linear navigation, required-leaf recall via descend, nodes never evidence, ns-homogeneous, deterministic packet) -> fold + close.** i34 = the hierarchy slice; the working-memory STORE + query ROUTER = i35.
- **SUNSET COUNTDOWN (mandate 01) -- iteration 34 -> sunset i40 = 6 iterations left.** `PROCESS_MANDATE.md` header `current_iteration`/`iterations_to_sunset`; at 0 -> REPORT_DUE + the mandatory metastability report BEFORE wave work. DECREMENT it each session (the machine-checkable countdown Nicholas asked not be forgotten).
- **i33 CLOSED (D-0097): NAMESPACE-CLOSURE + SUPERSESSION-HARDENING done** -- wave SHIPPED (#36 0.4.0, #37 selpol 1.2.0 / eval 0.5, #40 0.5.0; HEAD `c497d1b`) + the D-0077 MIXED-NAMESPACE fold smoke PASSED (fail_count=0: real #36->#40 packet valid + deterministic; a cross-ns candidate -> a SANITIZED fail-closed abort with ZERO leakage; namespace a closed set both ways; 0 orphans). ONE canonical `ns_permitted` (risk-6). **NEXT = i34 = MEMORY_ARCHITECTURE Tier 1** (the hierarchy TREE + node-synopsis + shortlist-and-descend + the working-memory STORE + the query ROUTER -- the Tier-0-reserved seams get their builds). --- dispatch detail: PART 1 (contracts) COMMITTED (`41410d8`) + mirrored -- MEMORY_CONTRACT **A5** + CONTEXT_PACKET_CONTRACT **i33 amendment** ADDITIVELY fold the frontier Tier-0 red-team (`159e9cb5`, D-0095) changes 1-5 + risk-6: namespace CLOSURE (one canonical `ns_permitted` enforced per-hop + every packet-visible object scope-checked + namespace-homogeneous derivations + sanitized fail-closed rejection + `effective_allowed = intersection(request,grant)` + one shared predicate/policy); candidate-INDEPENDENT `effective_current` from the catalog + a `superseded` s5 value + `superseded_by`/`supersedes` edges; provenance_mode-conditional hit shape + `candidate_role`/retrieval-stage lineage; the working-state STORE seam (state_version/CAS/one-active-head/promotion; continuity-authoritative; conjunctive task_id+namespace access; `search` rejects `working`); query_class-vs-temporal_intent split + a versioned classifier. Budgets raised (MEMORY_CONTRACT 28->38, CONTEXT_PACKET 25->29 KB). PART 2 the 3-lane CPU re-conformance wave (#36 0.3->0.4, #37 selpol 1.1->1.2 / eval 0.4->0.5, #40 0.4->0.5; plan `fo-33-d7b55e46`, dispatch_now 3 / 0 gpu / 0 doc-contention / clean preflight) DISPATCH-READY -- worker prompts delivered, slots 001/002/003 READY. **NEXT = Nicholas dispatches #36/#37/#40 -> poll `-Action status -PlanId fo-33-d7b55e46` -> `-Action handoff` -> the D-0077 MIXED-NAMESPACE fold smoke on the HARDENED contracts (now testing the leakage paths) -> fold + close (= PB-3 slim).**
...
- i33 `fo-33-d7b55e46` (D-0096, PART 1 committed + PART 2 DISPATCH-READY): the **NAMESPACE-CLOSURE + SUPERSESSION-HARDENING re-amendment** (folds the frontier Tier-0 red-team `159e9cb5`, D-0095 -- NO-GO to close i32 as-written). PART 1 (`41410d8`): MEMORY_CONTRACT **A5** + CONTEXT_PACKET_CONTRACT **i33 amendment** (namespace CLOSURE end-to-end; candidate-independent `effective_current` + `superseded`/`superseded_by`; provenance_mode hit shape + `candidate_role`; working-state store seam + search rejection; query_class/temporal_intent split) + a machine-checkable PROCESS_MANDATE sunset countdown + budgets (MC 38 / CP 29 KB). PART 2 (3-lane CPU, plan `fo-33-d7b55e46`, dispatch_now 3 / 0 gpu / 0 doc-contention): #36 0.3->0.4, #37 selpol 1.1->1.2 / eval 0.4->0.5, #40 0.4->0.5. SHIPPED + CLOSED (D-0097): #36 0.4.0 (`412d7fe`+`b754b61`), #37 selpol 1.2.0 / eval 0.5 (`37e5dd3`), #40 0.5.0 (`c497d1b`); the D-0077 mixed-namespace fold smoke PASSED fail_count=0 (airtight sanitized fail-closed closure; deterministic real #36->#40 packet; 0 orphans); NO divergence. HEAD `c497d1b`.
...
**i33 CLOSED (2026-08-04, D-0097): the re-conformance wave SHIPPED + the D-0077 mixed-namespace fold smoke PASSED (fail_count=0).** HEAD = **`c497d1b`** (chain `412d7fe` #36 0.4.0 -> `37e5dd3` #37 -> `b754b61` #36 predicate-align -> `c497d1b` #40 0.5.0, atop the D-0096/D-0097 doc commits). Modules: #36 artifact.search **0.4.0** (schema 3->4; per-hop + all-object namespace closure + sanitized fail-closed rejection + candidate-independent effective_current/`superseded`/`superseded_by` + provenance_mode hits + working store reservation; 179/179), #37 retrieval.eval eval **0.5** / selpol **1.2.0** (NEW lib/namespace_policy.py = the ONE canonical ns_permitted [risk-6] + lib/classifier_policy.py; pool-independent supersession; 195/195), #40 context.compiler **0.5.0** (effective=intersection(request,grant) strict fail-closed + all-object scope-check + sanitized abort + catalog effective_current + working_memory conjunctive + query_class/temporal_intent split; imports #37 READ-ONLY; 280/280). The D-0077 smoke (`runtime/smoke-i33b.ps1`) PASSED fail_count=0: (i) #40 on a REAL #36 catalog -> valid context_packet/0.2 + i33 fields + deterministic packet_id [#40's deferred -Live CLOSED]; (ii) a cross-ns candidate -> a SANITIZED `namespace_closure_violation` abort, ZERO identifying leakage, count only; (iii) namespace a closed set both ways; 0 orphans. No held `res.lease`; heartbeat `degraded:false`; 0 UNMANAGED llama-server/python. **PROCESS_MANDATE 01 countdown = current_iteration 33 / 7-to-sunset (i40); the next session (i34) decrements to 6.** **NEXT = i34 = MEMORY_ARCHITECTURE Tier 1** (s10): the bounded-fanout hierarchy TREE + node-synopsis + shortlist-and-descend + the per-task working-memory STORE + the multi-channel query ROUTER -- the Tier-0-reserved seams (node/working kinds, superseded/contradicts edges, candidate_role, the query-classification stub) get their real builds. **DOC-DEBT (PB-3, DELIBERATELY DEFERRED, deadline i40):** the hot docs stay over budget + CURRENT_STATE/MODULE_ROADMAP are stale at i30/i31 -- a consolidated slim + currency catch-up is due at the next doc-touch or when PB-1 lands. Slots 001/002/003 hold the i33 briefs -- archive + reset them at the i34 scoping.

**i33 BOX (2026-08-04, D-0096 -- PART 1 committed, PART 2 DISPATCH-READY; supersedes the i32 box below):** HEAD = **`41410d8`** (Docs D-0096: MEMORY_CONTRACT A5 + CONTEXT_PACKET_CONTRACT i33 amendment + PROCESS_MANDATE sunset countdown + DOC_PROTOCOL budgets [MC 38 / CP 29] + DECISION_LOG/INDEX; 6 files; native-git verified, no `add -A`). PART 2 plan **`fo-33-d7b55e46`** (3-lane CPU, GPU skipped; dispatch_now 3 / queued 0 / 0 gpu-serialized / 0 doc-contention / preflight clean, no held leases); worker prompts delivered; slots 001/002/003 READY (i32 briefs archived to `archive/fanout-agents/i32-*.md`). No LIVE `res.lease` held (durable `gpu-e3c5ba51.*` siblings persist by design -- NOT held); heartbeat `degraded:false`; 0 UNMANAGED llama-server/python. **PROCESS_MANDATE 01 countdown = 7 iterations to sunset (i40); address every session.** **NEXT = dispatch #36/#37/#40 -> `-Action status -PlanId fo-33-d7b55e46` until ready_for_handoff -> `-Action handoff` -> the D-0077 MIXED-NAMESPACE fold smoke on the HARDENED contracts: build a #36 catalog with ns-A + ns-B records + a superseded/successor pair whose successor is ABSENT from the top-k + a reserved `node` + a `working` record -> a ns-A `current_only` compile via #40 importing #37's shipped selpol 1.2.0 + `ns_permitted` -> assert ZERO ns-B leakage in evidence AND every diagnostic array, derived-record homogeneity rejection, pool-independent effective_current exclusion, working-state conjunctive access, node additive-ingest, every excerpt reconstructs to source, byte-identical #40-vs-direct-select, deterministic packet_id, 0 orphans -> fold + close i33 (snapshot+rewrite this handoff; PB-3 slim the over-budget hot docs).**

...
- **i47 CLOSED (D-0132/D-0133). HEAD = the i47 results commit (confirm `git log -1`).** NO code wave: (1) **mandate 02 SUNSET** -- the s3 report delivered, verdict **YES** (reverses mandate-01's NO); **SEALED_CHECK_47 armed** (core-docs/, 7 deterministic predicates + integrity hash; EVALUATE at the first session with iteration >= 54, after any due report, BEFORE wave work; live mandate doc DELETED, archived `archive/mandates/mandate-02-i40-i47.md`). (2) **the FROZEN legacy-vs-PCB migration gate EXECUTED** (T1; A/B Opus dry-runs -- B run-1 VOID on a wrong repo-grant, one clean re-run; blind KEEP-flip C1/C2 adjudication, swap-stable) -> **CONDITIONAL (D-0133)**: quality PASS (floors clear; B comprehension/constraint 4.0) but efficiency VOID -- the PCB-booted agent opened the legacy handoff for wave mechanics (RT2-F9; ALSO the central finding -> CD-1). Full record: `modules/44-project-map/eval/results/I47_RESULTS.md`. **THE LEGACY BOOTSTRAP (this doc) REMAINS DEFAULT.** **NEXT = i48: the CD path (s4 a) -- Nicholas's D-0134 direction** (anti-collapse: bounded-context memory + function with stability; audit acknowledged built-out, deferred). **Seat = Fable 5 until Nicholas declares settled (D-0134).** NO open Nicholas items: poser live-click CONFIRMED (9B explain works; defect: the explain window can't be closed after the fact -> next w08 touch), w07 DELETED, eval folder REMOVED. (Detail: s3 ledger.)
...
- **MANDATE 02 SUNSET at i47 (D-0132; verdict YES) -- NO live mandate exists.** The surviving controls: the M2-A commit gate (mechanical), deterministic PB triggers, cadence headers, the monitor, and **SEALED_CHECK_47** (evaluate ONLY at iteration >= 54; a pre-i54 session just leaves it sealed). M2-D verification-before-ratification stays standing practice.
...
**Now:** modules 0-34 + memory #35-#43 + **#44 project.map (PCB)** + widgets 01-08 built; 47 fan-out iterations run; Tier-1 ACCEPTED (i36); the P0-1 gate is a RATIFIED DESIGN pass (`p0_1_gate_status=pass`, D-0118; activation PROHIBITED); the doc-gate LIVE; `-AutoRamp` default-ON; the 9B GPU-resident on b10092; warm pool FROZEN (D-0079). **i47 = mandate-02 SUNSET (YES; sealed check armed) + the migration gate -> CONDITIONAL (D-0132/D-0133); i46 = the PCB ship (D-0130/31); i45 = LRAP + the poser arc (D-0122..29).**

**Iteration ledger** (one line each; detail = the D-entry; older lines in `archive/handoffs/`):

- **i1-i35 (D-0055..D-0100):** pre-memory infra + video arc (res.lease/frontier.bridge/executor/Governor/warm-pool GATE-NO/video spine/widgets 01-04) + the memory-substrate arc (Waves 1-3 = #35..#41, contract freezes, Tier-0 repairs) + the Tier-1 slice + NEW #42; detail in the D-entries + archive/handoffs/.
...
- **i47 (D-0132/33):** NO code wave -- mandate-02 SUNSET (report delivered, verdict YES -> SEALED_CHECK_47 armed, open i>=54; live doc deleted) + the FROZEN migration gate EXECUTED (T1; A valid; B run-1 VOID wrong-grant -> one clean re-run; blind KEEP C1/C2, swap-stable; efficiency VOID via RT2-F9) -> **CONDITIONAL**: legacy STAYS default; CD-1/2/3 bounded i48 units in `modules/44-project-map/eval/results/I47_RESULTS.md`.

Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` · artifacts `.../runtime/artifacts/<id>/` · leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 48.**

## 4. Current frontier -- NEXT = i48

**i47 delivered:** mandate-02 SUNSET (YES; SEALED_CHECK_47) + the migration gate = **CONDITIONAL** (D-0133; the full rule/score/proxy record is `modules/44-project-map/eval/results/I47_RESULTS.md`). **The legacy handoff stays the DEFAULT bootstrap until an i48 re-check passes.**

**i48 menu (D-0134: Nicholas picked (a); the rest deferred, not dropped):** (a) the CONDITIONAL path -- **CD-1** boot-surface wave-mechanics (extend BOOT_PACKET/L1 with the clamp/lease/ship-verify canon, pointer-backed, within the 20 KB budget; accept = a PCB-booted dry-run answers K3/K4/TR2/TR3 complete with NO tree-handoff open) -> **CD-2** the efficiency re-check (both s7 bars met un-voided) -> **CD-3** #44 query-surface (edge-id forms + alias resolution + provenance-at-SHA; accept = B run-2's three failed queries resolve); a passed re-check -> the migration unit (PCB default; FO-2; DOC_PROTOCOL rows; legacy = fallback until a retirement gate). (b) AUDIT_PIPELINE next_increment (front step + ride-along + OUTPUT side; design-first -> red-team; lease windows, P3/P6; review_due i49). (c) M2-C/FO-3 (map->#36 records; the INDEX at its cap = the standing signal). (d) #40 beam-WIDTH. (e) PB-2 (trigger-gated). **P0-1 ACTIVATION stays FROZEN** (no `non_execution=false` until Nicholas licenses). **SEALED_CHECK_47 is NOT evaluated before i54.**
...
## 11. Box state at handoff (2026-08-11, D-0133 -- i47 CLOSED)

**i47 closed: the sunset commit (`53c211f`) + the results/docs close commit (confirm `git log -2`).** NO wave/plan this session (no `fo-47-*`); the A/B/C sessions were eval-packet dispatches by Nicholas, not FANOUT_AGENT slots (slots EMPTY, unused). No held res.lease (durable `gpu-*.fence/.state/.txn` siblings persist by design); heartbeat `degraded:false`; 0 UNMANAGED orphans. Doc-gate PASS on both commits. i47 scratch PURGED (D-0134). **The eval folder is REMOVED (D-0134)** -- worktree removed + pruned (list = main only); every eval artifact lives byte-verbatim in `eval/results/`. A stale 0-byte `.git/index.lock` from a mount-side `git status` was cleared in-task (mount git = LOG-ONLY from now on). Open Nicholas items: NONE (D-0134; the w08 window-close defect is routed to the next w08 touch). **NEXT = i48** (s4 menu).

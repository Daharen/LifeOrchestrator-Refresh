...
**The i48 re-check RAN same-session via Nicholas's remote held-session loop and FAILED on the frozen s7 mechanics (D-0136; the full per-rule table: `modules/44-project-map/eval/results/I48_RECHECK_RESULTS.md`):** RT2-F9 VOID twice (B opened the legacy handoff un-routed for frontier detail; B total > 0.9x A) AND both bars missed -- B boot 18,724 vs bar 15,920 (arithmetically dead post-CD-1: the packet is 15,111 B BECAUSE it now carries the canon); B total 862,987 vs A 698,519 (**B = 1.24x A**; ~75% of B's ingest = raw-store fallback: harvest 478,784 + tool source 90,116 + arch json 82,474 -- deep narrative (purpose/SCHEMA_NOTES) is not query-reachable). Comprehension parity repeated (identical correct T3 increment, all traps hit, both arms); step-0 fail-closed proven in the field. **Legacy stays default. The i49 PCB path = N1 L2 narrative query surface + N2 overlay frontier richness + N3 packet query-verb table + N4 BAR RE-FREEZE (Nicholas ratification REQUIRED -- the frozen bars are dead) BEFORE any fresh gate.** Both A/B packs double as design input for the eventual #40 beam-width wave.

...
**i41 unit-0 (BLOCKING for the gate): fold the round-3 review.** When Nicholas returns the answer for pack `5807bc3e` (`modules/31-frontier-bridge/runtime/artifacts/5807bc3e-*/i40-p01gate-exactclosure-pack.answer.md`): run `read-return -ExpectPackId 5807bc3e` (expect captured/valid + pack_id_match), fold into a research digest. **On PASS:** re-ratify `ACTION_AUTHORIZATION_CONTRACT.md` s7 to `p0_1_gate_status=pass`, NAMING the pack id in the D-entry (M2-D). **On FAIL:** the findings become the next #43 unit; s7 stays walked-back; do NOT re-claim.
...
- **Frontier pack:** #31 `pack` takes `{prompt, question?, paths?}`; Nicholas couriers; the answer goes BETWEEN the two `<<<FRONTIER-BRIDGE-ANSWER-...>>>` markers (keep the `<!-- pack_id -->` line); `read-return -ReturnFile <path> -ExpectPackId <id>` -> fold into a research digest.

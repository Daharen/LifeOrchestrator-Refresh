...
- **i60 CLOSED (D-0156): bounded-ingest + control-plane hardening DELIVERED** -- 3-lane wave fo-60-d578e353 shipped A+B (`ops/retrieval/retrieve.ps1` + `gen-retrieval-monitor` `--gate`/`--artifacts-dir`, 88f41dee), C FO-6 (`Invoke-ProjectMap.ps1` `--repo` passthrough; #44 0.4.1; e4c4fdde), D bounded MANAGER_VIEW (`ops/manager`, f838952a); cross-lane smoke green; E SEALED_CHECK_47 integrity intact + SP3 FAIL -> seal retained, M-03 NOT licensed. **NEXT = i61 = bounded-ingest ADOPTION proof + hardening** (F-i53-eff OPEN; fail-closed retrieval gate WIRED into the close path -- D-0157, ops/close-refold.ps1 -Ledger, zero-floor active i60; first class migration -> i68). Slots 001/002/003 archived (i60-00N) + RESET EMPTY. AUDIT review_due i62. Seat = Opus 4.8 (settled).
...
Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` * artifacts `.../runtime/artifacts/<id>/` * leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 61.**

## 4. Current frontier -- i60 CLOSED (D-0156): NEXT = i61 = bounded-ingest ADOPTION proof + hardening; AUDIT review_due i62; SP3/M-03 open (i60 recorded, M-03 NOT licensed)

**i59 CLOSED (D-0154, the ROOT front-door migration) + RECONCILED (D-0155).** The migrated root -- START_HERE boot KERNEL + `COLD_BOOT_CARD.md` (generator-independent survivor) + `ops/frontdoor/` uniform rebuild interface + the migration gate -- is detailed in the s3 ledger + D-0154. **NEXT = i60 = bounded-ingest + control-plane hardening (D-0155; NOT a class migration):** prove + ENFORCE bounded bootstrap/retrieval on fresh work (close **F-i53-eff** -- bounded `card:`/`section:` the easy default, a whole-doc open needs an explicit reason), make retrieval measurement automatic, repair **FO-6**, produce a bounded generated **manager/program projection**, and treat SP3/M-03 + stale version/test claims as explicit TRUTH questions -- **i60 may INVESTIGATE the SP3/M-03 truth+budget but must NOT silently license M-03** (P0-1 activation stays FROZEN). The first PB-7 class migration DEFERS to **i68** (after bounded-ingest is adopted + the close transaction lands). Rolling i60-i79 roadmap: `research/2026-08-15-i59-roadmap-reconciliation.md` (i60-61 bounded-ingest / i62-67 close transaction / i68-73 semantic-family front doors / i74-76 memory-in-loop / i77-79 inspectability). AUDIT review_due i62 NON-DISPLACING; SEALED_CHECK_47 retained; #40 beam-width / PB-2 / w08 deferred (D-0134).

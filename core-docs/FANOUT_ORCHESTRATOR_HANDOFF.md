...
- **i50: the N4 migration-gate BAR RE-FREEZE is RATIFIED (D-0140, Nicholas). HEAD = the i50 docs-close commit.** The frozen `I47_EVAL_PACKET` s7 efficiency rules (dead post-CD-1, D-0136) are REPLACED for the next gate run: **BOOT** absolute (`BOOT_PACKET <= 20,000 B` + CD-1 canon, degrade-LAST; retires the unreachable 0.5xA ratio); **TOTAL** `B <= 0.7 x A` counting query-OUTPUT bytes for #44-query content (whole-file only for raw opens; credits N1); **VOID** on PCB-insufficiency (corroboration counts but does not void; binary RT2-F9 + void-(b) retired). s1-s6 + quality floors UNCHANGED; packet untouched (record: `modules/44-project-map/eval/results/N4_BAR_REFREEZE.md`). **A fresh legacy-vs-PCB gate is now STAGEABLE but was NOT run in i50; the LEGACY BOOTSTRAP (this doc) STAYS DEFAULT until it runs + passes** (projected total ~0.35xA). i49 (D-0138) shipped #44 0.3.0 (N1 narrative queries + N2 frontier + N3 verb table; fold VALIDATE 0). **Seat = Fable 5 until Nicholas declares settled (D-0134).**
...
Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` · artifacts `.../runtime/artifacts/<id>/` · leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 51.**

## 4. Current frontier -- NEXT = i51 (stage the fresh PCB migration gate, or a deferred-menu pick)

**i50 RATIFIED the N4 bar re-freeze (D-0140):** the migration-gate s7 efficiency rules are re-frozen -- BOOT absolute (`BOOT_PACKET <= 20,000 B` + CD-1 canon, degrade-LAST), TOTAL `B <= 0.7 x A` counting query-OUTPUT bytes for #44-query content (whole-file only for raw opens), VOID on PCB-insufficiency (corroboration counts but does not void); the 0.9xA void-(b) + the 0.5xA boot ratio retired; s1-s6 + quality floors unchanged; packet untouched (record: `modules/44-project-map/eval/results/N4_BAR_REFREEZE.md`; digest: `research/2026-08-12-i50-n4-bar-refreeze-proposal.md`). **A fresh legacy-vs-PCB gate is now STAGEABLE against these pre-frozen bars but was NOT run in i50 -- the LEGACY handoff stays the default bootstrap until a future iteration stages + runs it and it PASSES** (projected total ~0.35xA; i49 closed F1/F3/F4 at query granularity -- purpose ~5.7 KB, section ~7.7 KB vs the 478,784 B grep). Both i48 A/B packs double as design input for the eventual #40 beam-width wave.

**Deferred menu (D-0134, deferred NOT dropped):** (b) AUDIT_PIPELINE next_increment (front step + ride-along + OUTPUT side; design-first -> red-team; **review_due i54, D-0137 override**; + the w08 explain-window-close defect rides the next w08 touch, D-0064 live-GUI cycle) · (c) KNOWLEDGE SURFACE RE-LAYER (D-0141/PB-7, subsumes PB-6): cumulative registries -> bounded-hot + indexed-cold on the PCB (horizontal) + memory (vertical) engines; a cap = a re-layer trigger, not endless compression · (d) #40 beam-WIDTH · (e) PB-2 (trigger-gated). **P0-1 ACTIVATION stays FROZEN. SEALED_CHECK_47 is NOT evaluated before i54.**
...
## 11. Box state at handoff (2026-08-12, D-0140 -- i50 N4 RATIFIED)

**i50: NO code wave -- the N4 bar re-freeze RATIFIED (D-0140); HEAD = the i50 docs-close commit.** No worker plan. Landed: D-0140 + index row; NEW `modules/44-project-map/eval/results/N4_BAR_REFREEZE.md` + `research/2026-08-12-i50-n4-bar-refreeze-proposal.md`; handoff s0/s3/s4/s11 + CURRENT_STATE refreshed; outgoing handoff snapshot archived. No held res.lease (durable `gpu-*` siblings persist by design); heartbeat `degraded:false`; 0 UNMANAGED orphans. Doc-gate PASS. Slots EMPTY. **Open Nicholas item: the i51 pick -- stage the fresh PCB migration gate (now licensable), or a deferred-menu item (#40 beam-width / M2-C decision re-layer PB-6 / AUDIT @i54 / PB-2).** The w08 explain-window-close defect stays routed to the next w08 touch (D-0134).

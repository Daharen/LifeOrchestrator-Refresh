...
- **i48 CLOSED (D-0135). HEAD = the i48 fold commit (confirm `git log -2`: docs-close then fold).** The CD-path wave `fo-48-3d3a4e1b` (ONE coding lane) shipped **#44 `project.map` 0.2.0** = the D-0133 CONDITIONAL code closures: **CD-1** (a claims-sourced, pointer-backed OPERATIONS section in the BOOT_PACKET -- wave-clamp/lease/ship-verify/orphan/D-0077 canon; degrade-LAST + min-floor) + **CD-3** (short-form/alias id resolution with full-id byte-identity + evidence beyond-tree marking + the currency block). **ORCHESTRATOR-RECOVERED ship `c451890`** (the i40 pattern: the worker session had no pwsh/exec-job.sh/res.lease; its files landed via its device bridge; independently verified 93/93 on box py3.12 + probes before ship) **+ the harvest sunset-fix `4055dc1`** (orchestrator-inline: 0.1.0 harvest hard-read the i47-sunset-deleted PROCESS_MANDATE.md and CRASHED on the live tree; absent doc now = empty header {}; an overlay claiming a mandate against an empty header fires OVERLAY_MANDATE_DRIFT; suite 93->98). **FOLD: VALIDATE 0 findings** (reaffirms + prunes + new entities + the i48 no-mandate overlay; 2 recorded fold corrections -> `claims/i48-ops-canon-claims-foldfix.json`; detail D-0135); render + `-Live Folded` green in the fold commit. **NEXT = the CONDITIONAL re-check with Nicholas (s4): CD-1 probe-only dry-run, then CD-2 A/B efficiency re-run.** **THE LEGACY BOOTSTRAP (this doc) REMAINS DEFAULT until the re-check passes.** **Seat = Fable 5 until Nicholas declares settled (D-0134).**
...
- Workers use `docs:[]`; YOU mirror core-docs under the `git` lease (s7); doc rules in `DOC_PROTOCOL.md`. Deliver prompts/packets/packs to Nicholas as FILES; briefs also go in `core-docs/fanout/FANOUT_AGENT_00N.md` (s5). Slots 001/002/003 EMPTY (003 filled + archived `i48-003` + reset at the i48 close).
...
**Now:** modules 0-34 + memory #35-#43 + **#44 project.map 0.2.0 (PCB)** + widgets 01-08 built; 48 fan-out iterations run; Tier-1 ACCEPTED (i36); the P0-1 gate is a RATIFIED DESIGN pass (`p0_1_gate_status=pass`, D-0118; activation PROHIBITED); the doc-gate LIVE; `-AutoRamp` default-ON; the 9B GPU-resident on b10092; warm pool FROZEN (D-0079). **i48 = the CD closures + fold (D-0135); i47 = mandate-02 SUNSET (YES) + the migration gate CONDITIONAL (D-0132/33); i46 = the PCB ship (D-0130/31).**
...
- **i48 (D-0135):** the CD-path wave `fo-48-3d3a4e1b` (1 coding lane, Opus) -- #44 **0.2.0**: CD-1 OPERATIONS boot canon + CD-3 query surface -- **ORCHESTRATOR-RECOVERED ship `c451890`** (worker session had no pwsh/executor; files independently verified pre-ship) + **the harvest sunset-fix `4055dc1`** (inline; 93->98) + fold (reaffirms/prunes/+6 entities/overlay i48/2 corrections) -> **VALIDATE 0**; render + `-Live Folded` green (fold commit); **the re-check staged, not yet run**.

Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` · artifacts `.../runtime/artifacts/<id>/` · leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 49.**

## 4. Current frontier -- NEXT = the i48 re-check, then i49

**i48 delivered the CD closures; the CONDITIONAL re-check itself is STILL OPEN** and is the FIRST work of the next session (it needs Nicholas dispatches; no new wave before it resolves):

1. **CD-1 acceptance dry-run (probe-only; consumes NO pre-registered task):** fresh frozen worktree + eval folder (i47 pattern; EVAL_SHA_2 = the i48 fold commit) + a `_bundle` rendered at that SHA; ONE PCB-booted dry-run session (B-protocol boot; eval-folder grant ONLY) answers K3/K4/TR2/TR3 pointer-cited. **Accept: complete answers + NO `tree\` legacy-handoff open in its ledger (RT2-F9 un-triggered).** Fail -> fix boot surface, repeat before spending CD-2.
2. **CD-2 efficiency re-check:** the FROZEN I47_EVAL_PACKET s7 mechanics at EVAL_SHA_2 -- Nicholas picks from the UNUSED T2/T3 at dispatch (records choice+time); A + B dry-runs (Opus 4.8 Extra both; eval-folder grant only; one re-run per void, s5 rule); proxies orchestrator-computed. **Accept: BOTH bars met un-voided (boot <= 0.5x A narrow; total <= 0.7x A; void on a non-routed legacy-handoff open OR total > 0.9x A).** Quality PASSED at i47 -- NO C round.
3. **On pass -> the migration unit (i49):** PCB becomes the default orchestrator bootstrap; FO-2 (CURRENT_STATE.json); DOC_PROTOCOL promotion rows; THIS doc becomes fallback/history until a retirement gate. On fail -> <=bounded corrections per D-0133's pattern or NO-GO recorded.

**Deferred menu (D-0134, deferred NOT dropped):** (b) AUDIT_PIPELINE next_increment (front step + ride-along + OUTPUT side; design-first -> red-team; **review_due i49**; + the w08 explain-window-close defect rides the next w08 touch, D-0064 live-GUI cycle) · (c) M2-C/FO-3 (map->#36 records; the INDEX at its 20 KB cap = the standing signal) · (d) #40 beam-WIDTH · (e) PB-2 (trigger-gated). **P0-1 ACTIVATION stays FROZEN. SEALED_CHECK_47 is NOT evaluated before i54.**
...
## 11. Box state at handoff (2026-08-11, D-0135 -- i48 CLOSED)

**i48 closed: `c451890` (recovered 0.2.0 ship) -> `4055dc1` (harvest fix) -> the docs-close commit -> the fold commit (map+generated; confirm `git log -2`).** Plan `fo-48-3d3a4e1b`: 1 worker, report filed (hand-written; recovery recorded in `c451890` + REPORT-PCB-CD-i48.md). No held res.lease (durable `gpu-*` siblings persist by design); heartbeat `degraded:false`; 0 UNMANAGED orphans. Doc-gate PASS on the docs commit. Slots EMPTY (003 archived `i48-003`). `modules/44-project-map/eval/` holds the I47 packet + results (the i47 worktree/folder were REMOVED at D-0134; the re-check stages a FRESH one at EVAL_SHA_2). **Open Nicholas items: the CD-1 probe dispatch + the CD-2 task pick (T2/T3) -- the next session's FIRST work (s4).** The w08 explain-window-close defect stays routed to the next w08 touch (D-0134).

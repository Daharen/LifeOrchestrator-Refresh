...
- **SUNSET COUNTDOWN (mandate 01) -- iteration 33 -> sunset i40 = 7 iterations left.** `PROCESS_MANDATE.md` header `current_iteration`/`iterations_to_sunset`; at 0 -> REPORT_DUE + the mandatory metastability report BEFORE wave work. DECREMENT it each session (the machine-checkable countdown Nicholas asked not be forgotten).

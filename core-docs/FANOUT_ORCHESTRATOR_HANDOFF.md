...
- **i53: the fresh two-class migration gate RAN -> GO (D-0146). The PCB `modules/44-project-map/generated/BOOT_PACKET.md` is now the DEFAULT orchestrator bootstrap; THIS doc is the FALLBACK + deep-reference + Project-mirror context (boot from the packet first; drop here when it points you here or is unreachable). B(PCB) beat A(legacy) on BOTH task classes -- straight 4.0 vs multiple 2.0s; N6's overlay canon (prohibitions + open_rulings rendered into the packet) closed the i51 constraint gap; N5/N7 cleared the retrieval + currency deficits. Evidence: `eval/results/I53_RESULTS.md` (commit `12b32f5`). Finding F-i53-eff: prefer `section:`/`card:` fetches over whole-doc opens on prose tasks. Seat = Fable 5 until Nicholas declares settled (D-0134). Next = i54 (SEALED_CHECK_47 opens i>=54 + the AUDIT_PIPELINE review).**
...
- Workers use `docs:[]`; YOU mirror core-docs under the `git` lease (s7); doc rules in `DOC_PROTOCOL.md`. Deliver prompts/packets/packs to Nicholas as FILES; briefs also go in `core-docs/fanout/FANOUT_AGENT_00N.md` (s5). Slots 001/002/003 EMPTY (003 filled + archived `i52-003` + reset at the i52 close).
...
- **i46-i47 (D-0130..33):** the PCB wave `fo-46-6dd32d37` built modules/44 **0.1.0** (D-0077 fold; the I47 gate packet FROZEN) -> mandate-02 SUNSET (SEALED_CHECK_47 armed, open i>=54) + the frozen gate EXECUTED = **CONDITIONAL** (legacy default; CD-1/2/3 in `eval/results/I47_RESULTS.md`).
- **i48 (D-0135/36):** the CD wave `fo-48-3d3a4e1b` -- #44 **0.2.0** (CD-1 canon + CD-3 queries; ORCHESTRATOR-RECOVERED ship `c451890` + the inline sunset-fix `4055dc1`); fold VALIDATE 0; **re-check FAILED same-session (D-0136: void x2, B 1.24x A)**.
- **i49 (D-0138):** the N1 wave `fo-49-655c25df` (1 coding lane, Opus) -- #44 **0.2.0 -> 0.3.0**: N1 narrative query surface (purpose + SCHEMA_NOTES section) + N2 overlay frontier richness + N3 verb-table (single-source, dispatcher-asserted); ship `5d46731`; fold VALIDATE 0 (7 reaffirms + version-sync + iteration-49 overlay) + render 16,640 B + `-Live Folded` green (fold `c0c3abe`); independent replays reproduced. Legacy stays default; N4 ratified in i50 (D-0140).
- **i50 (D-0140):** NO code wave -- N4 migration-gate BAR RE-FREEZE ratified (Nicholas): the frozen s7 efficiency rules replaced by absolute boot (`packet <= 20,000 B` + CD-1 canon) + total `B <= 0.7 x A` on query-output accounting + void on PCB-insufficiency (void-(b) + 0.5xA boot ratio retired); a fresh gate is stageable (NOT run); legacy stays default. New `N4_BAR_REFREEZE.md` beside the I47/I48 eval results.
- **i51 (D-0142):** NO code wave -- the FRESH gate RAN (T1; full A/B/C1/C2; EVAL_SHA `ef70a57`; SWAP): **NO-GO** -- floor (b) breached (B constraint 3.0 v 4.0) + TOTAL 2.74x/1.12x A vs 0.7x (BOOT PASS; VOID clean); C-tier opus-5 recorded (A/B pin held); artifacts committed byte-verbatim; hot-doc staleness fixes landed at close.
- **i52 (D-0143..45):** the fix wave `fo-52-db941ec2` (1 Opus coding lane) -- #44 **0.3.0 -> 0.4.0** ship `c6c58a8` (169/169 x4 + negatives 33/33; N5 replays 2,660/2,359/1,659 B vs 20,156 + 2,494 vs 9,811 + 1,446 vs 10,209 + cards ~1.6 KB vs 31,488; N6 canon claims + open_rulings render; honest pre-fold -Live FAIL = the F2 stale set); **N7 ADOPTED + first close-refold run (the final i52 commit = the fold); N8 RATIFIED AS AMENDED (D-0144)**.

Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` · artifacts `.../runtime/artifacts/<id>/` · leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 52.**

## 4. Current frontier -- NEXT = i54 (SEALED_CHECK_47 opens i>=54 + the AUDIT_PIPELINE review); i53 = the migration gate GO (D-0146) -- the PCB is now the default bootstrap

**i52 closed the D-0142 fix set: N5-N7 LANDED, N8 RATIFIED (D-0144).** The fresh migration gate is STAGEABLE: run it under the D-0140/N4 bars + the D-0144/N8 protocol -- author + FREEZE the two-task pool (T-map map-native + T-prose prose-governing) at staging; re-derive the K/A keys against the staging tree BEFORE dispatch (adaptations recorded at staging, never at adjudication); C1/C2 pin = **Opus 4.8 Extra**, runtime mismatch = record + **STOP pre-C** (Fable 5 escalation only pre-frozen at staging; Opus 5 never); every arm ledger records **retrieval/materialization bytes SEPARATE from model-visible bytes** (the 0.7x bar runs on charged retrieval bytes; counters never collapsed); dispatches carry the frozen listing + staged-not-read disclosure lines; stage at HEAD (N7 keeps the map in-sync -- a stale map at staging is an N7 FAIL and blocks the gate). **GO requires every bar met on BOTH tasks**; anything less stays CONDITIONAL naming the failed class. Authorities: `eval/results/N8_RERUN_REFREEZE.md` + `N4_BAR_REFREEZE.md` + `I51_RESULTS.md`.

**Deferred menu (D-0134, deferred NOT dropped):** (b) AUDIT next_increment (front step / ride-along / OUTPUT side; design-first -> red-team; **review_due i54, D-0137**; the w08 window-close defect rides the next w08 touch) · (c) KNOWLEDGE SURFACE RE-LAYER (D-0141/PB-7, subsumes PB-6): cumulative registries -> bounded-hot + indexed-cold on the PCB + memory engines · (d) #40 beam-WIDTH (i48 + i51 packs = design input) · (e) PB-2 (trigger-gated). **P0-1 ACTIVATION stays FROZEN. SEALED_CHECK_47 is NOT evaluated before i54.**
...
## 11. Box state at handoff (2026-08-14, D-0145 -- i52 fix wave closed)

**i52: 1-lane wave `fo-52-db941ec2` DONE.** Ship `c6c58a8` (#44 0.4.0; dev.ship green; native-verified; report mirrored `claude/fanout/i52-PCB-N5N6-i52-REPORT.md`); N8 ratified `27b927e` (D-0144); close docs = this rewrite's commit; **the FINAL i52 commit = the first N7 fold (reaffirms + version-sync 0.4.0 + overlay i52 + canon ingest -> validate 0 -> render + -Check -> -Live Folded + independent replays + fresh-subagent canon probe -- all required green BEFORE that commit; its message + `modules/44-project-map/runtime/close-refold/` envelopes carry the numbers; verify at HEAD = 0 stale).** No held res.lease (durable `gpu-*` siblings persist by design); heartbeat `degraded:false`; 0 UNMANAGED orphans; doc-gate PASS on every close commit. Slots EMPTY. Eval folders + quarantine + i48/i51 worktrees on disk pending Nicholas disposition. **Open Nicholas item: pick i53 -- stage + run the fresh two-class gate (s4), or the deferred menu.** The w08 window-close defect stays routed to the next w08 touch (D-0134).

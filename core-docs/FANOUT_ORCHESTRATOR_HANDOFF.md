...
## 4. Current frontier -- NEXT = i58 (Nicholas picks); i57 CLOSED (D-0151): PB-6 boot-wiring SHIPPED -> PB-6 COMPLETE; AUDIT review_due i58; SP3/M-03 open

**i57 CLOSED (D-0151).** PB-6 boot-wiring SHIPPED (`d16cdc0`) -> **PB-6 COMPLETE**: `ops/refresh-decision-catalog.py` (standing #36 decision catalog refreshed each close) + the #44 overlay `standing_constraints` ROOT view (asserted_count + hot/enforced split + categories + spill-to-cold) + the BOOT_PACKET `STANDING CONSTRAINTS` line + #40 bounded pool load + the catalog-path gate (D5 24/24). Boot stops whole-ingesting `DECISION_LOG_INDEX` (D3, START_HERE). Real 653 KB log: asserted_count=94 == an independent count (0 drop, F1); BOOT_PACKET 18,101 B < 20,000 (G1); P0-1 non_execution untouched (G6); orchestrator independent re-verify (D5 24/24, i56 30/30, #44 169/169). **NEXT = i58 (Nicholas picks):** the AUDIT increment (review_due i58, D-0147) * PB-7's next cumulative surface (design target B/C: CURRENT_STATE sub-registries / the index-of-index) * boot-wiring named follow-ons (a #36 query-level bounded load; the overlay's static plane-map; the `ops:boot-decision-retrieval` OPERATIONS-canon entity). SP3/M-03 stays OPEN (Nicholas-deferred); P0-1 activation FROZEN; SEALED_CHECK_47 retained; #40 beam-width / PB-2 / w08 deferred (D-0134).

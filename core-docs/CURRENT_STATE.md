...
**i36 TIER-1 ACCEPTED + closed (D-0102; plan `fo-36-1a676e4b`).** Three CPU lanes shipped (all named-files-only, 0 orphans, no fold reconciliations): **Lane A** #37 retrieval.eval eval 0.8.0 (`0e466bc`; opt-in `wired_descend` drives #40 0.7.0's PUBLIC artifact_search descend port READ-ONLY + measures s10 against the WIRED packets; 55/55) + **Lane B** #36 artifact.search 0.6.0 (`16c27a8`; the clean `get-record` by-rvid READ op, closing i35's fold reconciliation; 38/38 + 227/227 live) + **Lane C** NEW widgets/05 Provenance Map (`3ad71d3`; the D-0101 audit tier-A1 read-only construction map; 100/100 + STA 8/8). **The orchestrator ran the FULL wired-descend Tier-1 acceptance gate** at fold -- harness 0.8.0 x the FROZEN #40 0.7.0 over a DISTINCT, hash-verified 6-package foreign corpus (637 files, 6 namespaces) at a 100x leaf span -> **11/11 s10 criteria PASS, 0 fold reconciliation -> project `tier1_accepted` FLIPPED TRUE** (never a silent pass; evidence `research/2026-08-05-i36-tier1-acceptance-rehearsal.md` + the committed report json). The i34 `smoke-i34.py` 38/38 regression held. **HONEST caveat (a follow-on, NOT a blocker):** the descend FAST-BEAM is bounded-beam LOSSY (`hierarchy_path_recall=0`) -- the indexed #36-flat fallback preserves packet + guaranteed recall (the safe-pruning design); strengthening the shortlist/descend beam so the hierarchy fast-path itself contributes recall is the next quality lever.

**NEXT = i37 (the #40 consumer GLUE + the P0-1 safety track, now unblocked):** (1) wire #40's packet working_memory region to #42's `get_active_head` (today "reserved; empty"); (2) the multi-channel query ROUTER (query_class is a stub) -- **R-1 (D-0101) BINDS: the router must be born instrumented with the deterministic integer-only versioned STAGE-TRACE, asserted by the D-0077 fold** -- these two are BOTH #40 (sequence them, one per wave); (3) FREEZE the action-authorization contract (proposal / tool_manifest / action_permit / completion) from the COURIERED frontier answer (`research/2026-08-05-i36-action-authz-freeze-frontier.md`, a CONDITIONAL-GO freeze-ready spec) THEN build the P0-1 adversarial-injection SUITE against it (design + digest in hand); (4) the FAST-BEAM recall follow-on (strengthen the descend/shortlist beam ranking). Nothing is action-capable (`non_execution:true` holds). Candidate menu + work orders: **`FANOUT_ORCHESTRATOR_HANDOFF.md`** section 4. **Audit program (D-0101/PB-4):** Widget 05 = tier A1 SHIPPED (its human live-GUI confirm is the one OPEN follow-on); Widget 06 Compile Trace Console is next (i37-i38); the cadence header in `research/2026-08-05-audit-pipeline-target.md` now reads last_reviewed i36 / review_due i39.
...
**Last updated:** 2026-08-05 -- i36 TIER-1 ACCEPTED + CLOSED (D-0102; plan `fo-36-1a676e4b`): Lane A #37 eval 0.8.0 (`0e466bc`, wired-descend drive) + Lane B #36 0.6.0 (`16c27a8`, get-record by-rvid) + Lane C NEW widgets/05 Provenance Map (`3ad71d3`, D-0101 audit A1). The orchestrator ran the FULL wired-descend rehearsal vs the FROZEN #40 0.7.0 over a DISTINCT 6-package foreign corpus at a 100x leaf span -> 11/11 s10 criteria, 0 fold reconciliation -> **project tier1_accepted FLIPPED TRUE** (honest caveat: descend fast-beam lossy, the #36-flat fallback preserves recall -- a follow-on). The action-authz freeze design is in hand (couriered frontier CONDITIONAL-GO). NEXT = i37: the #40 working_memory + router (R-1) glue + the action-authz freeze -> P0-1 suite. (Doc debt: hot docs still over budget -- PB-3 slim remains a named unit.)

...
- **Active module:** _none in progress._ **Module 0 job-runner SHIPPED (D-0048): `Invoke-DevShip.ps1` + `exec-job.sh`** -- one executor job gate+commits a unit (sha + AST + tests, fail-closed) and returns a compact JSON; dogfood-committed by dev.ship itself (`5644b9ba`), 27/27 + 24/24. **Next unit = Phase B Widget #2 -- Module Launcher / Registry Browser** (`MODULE_ROADMAP.md -> Build priority`). **Modules 0–25 complete** (0 executor · 1 `skill.bootstrap` · 2

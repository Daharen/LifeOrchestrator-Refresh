...
- **Adaptive Resource Governor (agent.local #21) -- Phase 1 DONE (D-0043):** the local DECISION floor was raised from the `[tiny,weak,mid]` ladder to a **mid-only floor**. Measured root cause (m30-exp3-001): the low-floor escalator ladder makes the competent 3B JUDGE the weak tiers' answer and ANCHOR on it (both runs accepted at `mid`, yet the ladder looped `gen.image` 6 steps/140 s and never finished; mid-only finished 3 steps/clean). Live default now: dog goal `-Route` -> completed 3/8 steps / **48 s** / one gen.image / **dog on the real Desktop** (`m31-p1-default-001`). Added a **`-Profile` knob (frugal|floor|max)** = the governor's rungs, **un-refused the 27B** in `route.tools` (soft-warn, not throw), default `max_steps` 4->8. `max` = mid decisions + **27B generation** (escalating the DECISION classifier to the 27B re-broke it with empty output, `m31-p1-max-001` -- a Phase 3 concern). **Phase 2 (warm server) is DONE (D-0057)** and **Phase 3 (auto-ramp) Stage-1 is DONE (D-0059** -- an opt-in `-AutoRamp` controller on agent.local #21: monotonic model-affine epochs M0->M1->S0 closed by a pre-frozen `lifeorch.goal_verification/0.1` contract; calibration confirmed the resident 9B decides 6/6 vs the 3B's 4/6). Phase 3 Stage-2 SLICE is DONE (D-0060, iteration 7): a deadline-gated X0/27B one-shot recovery rung (opt-in -AllowLegacy27B; LIVE fired -> verified_success at epoch X0) + an opt-in logprob-entropy soft signal (clean per-token logprobs confirmed on BOTH engine builds b8661 + b10092); -AutoRamp also wired into the Local Agent Console (widgets/01) as an opt-in toggle. Stage-2 remainder (self-consistency, pattern-learning, a fitting 27B quant, wiring -AutoRamp default-on) is deferred to iteration 8. **UPDATE (D-0061, iteration 8):** the 27B/X0 quant is validated IMPRACTICAL (no quant fits 11 GB GPU-bound; the resident 9B is the top rung) and -AutoRamp default-on was TRIED + REVERTED (contract-less runs loop+error without a success contract -- the controller ignores `finish`; fix = honor finish / apply the D-0046 terminator in the contract-less M0 path, iteration 9). A couriered frontier report recommends upgrading the 9B strong tier to Qwen3.5-9B-Q5_K_M (fidelity + KV headroom). See `ADAPTIVE_RESOURCE_GOVERNOR.md`.
- **Strong tier is now Qwen3.5-9B, fully GPU-resident (D-0044).** Replaced the split-brain 27B (partial offload, ~2 tok/s, timed out at 30 min) with **Qwen3.5-9B Q5_K_M** (strong tier upgraded Q4_K_M -> Q5_K_M for fidelity + KV headroom, D-0062; Q4 retained as `llm.strong.qwen3p5-9b-q4` wired:false for one-flip rollback; live S0 6/6 @2048 tok) -- loads fully on the RTX 2080 Ti at **~7.1 GB VRAM** (ngl 99, ~4.3 GB headroom) -> **GPU-bound ~68 tok/s**, clean terse JSON output. It is a hybrid attention-SSM arch the default engine b8661 cannot load, so a side-by-side **llama.cpp b10092 (CUDA 12.4, self-contained)** was staged and the 9B entry pins `engine_path` to it (every other tier stays on b8661 -- zero blast radius). A general gateway **`no_think`** hook (append `/no_think`) makes the 9B emit non-thinking output (default flags left reasoning ON -> empty). Strong is GENERATION-only; decisions stay at the mid floor. **Residual (D-0032) -- RESOLVED (D-0046):** a deterministic terminator + repeat-action guard in agent.local now blocks a premature `finish` until every routed planned tool has succeeded (and never re-runs a done tool). Live: the dog goal `-Route` at the FLOOR default landed on the real Desktop 3/3 (the terminator fired once, forcing fs.manage after the model tried to finish early); 87/87 tests. NEW residual: `-Profile max` still does NOT land -- the 9B (gen_tier=strong) arg-gen returns non-JSON (`arg_parse_failed`) every step so tools never get valid args (a 9B/gateway defect, NOT the terminator; floor with gen_tier=mid is reliable). See D-0046. The 27B entry is retained (reachable via -Model) but is no longer the tier. See D-0044 + ADAPTIVE_RESOURCE_GOVERNOR.md.
- **Active module:** **FAN-OUT loop through ITERATION 9 (D-0062) -- -AutoRamp DEFAULT-ON live (the D-0061 contract-less closing is FIXED) + 9B strong tier upgraded to Q5_K_M; [prior iter 8 (D-0061)] -- adoption+practicality: -AutoRamp default-on TRIED+REVERTED (contract-less regression) + the 27B/X0 quant NEGATIVE RESULT (no fit); the resident 9B is the top rung**; iter 8 (fo-8-e6f2d44f, 2 workers, 0 conflicts, 2/2) -- AR8-default-on flipped agent.local's default to -AutoRamp (278c088, mock gates green) but the orchestrator's live floor-check caught a contract-less regression (the controller ignores `finish` without a success contract -> loops to max_steps + status=error; the strict floor finishes in 1 step) so it was REVERTED byte-exact (fbacf69; opt-in default restored; controller+flag+contract path remain opt-in). X0-quant returned a NEGATIVE RESULT (no Qwen3.5-27B quant fits GPU-bound on 11 GB -> the resident 9B Q4 S0 stays the effective top, X0 opt-in+deadline-gated; nothing shipped). Nicholas couriered a ChatGPT deep-research model-selection report (digest core-docs/research/2026-07-28-frontier-local-model-selection.md) confirming the 27B is impractical + recommending Qwen3.5-9B-Q5_K_M (7.11 GB) as a fidelity upgrade over our Q4_K_M. Next = iteration 9 (fix the contract-less auto-ramp closing then re-enable default-on; stage 9B Q5_K_M; optional warm multi-model pool). **[prior active D-0060]** **FAN-OUT loop through ITERATION 7 (D-0060) -- Governor Phase 3 Stage-2 slice (deadline-gated X0/27B recovery + logprobs) + -AutoRamp wired into the Local Agent Console (adoption) SHIPPED**; iter 7 (fo-7-5a4c227d, 2 workers, 0 conflicts, 2/2) shipped AR-console-autoramp (widgets/01 opt-in Auto-ramp toggle + governor-trace render, 33da9a5, off-machine 85/85 + CPU-live 90/90) + S2-x0-logprobs (clean per-token logprobs on BOTH engine builds + a deadline-gated X0/27B one-shot recovery rung -AllowLegacy27B + an opt-in logprob-entropy signal, 830efcc, 83/83; LIVE X0 FIRED -> verified_success at epoch X0, 1 swap into the 27B, 0 orphans). Post-ship: the live-GUI toggle threw a null-ref ('Enabled' on $null from bare-local WinForms handlers) -- FIXED with .GetNewClosure() + a SelfTest toggle guard (b1f36f0, live 91/91); Nicholas re-confirmed the GUI toggle + a real auto-ramp run render cleanly (the Local Agent Console live-GUI adoption pass DONE). Next = iteration 8 (wire -AutoRamp into a DEFAULT caller; Stage-2 remainder self-consistency/pattern-learning; a fitting 27B quant; warm-server to other model modules). **[prior active D-0059]** **FAN-OUT loop through ITERATION 6 (D-0059) -- Governor Phase 3 Stage-1 auto-ramp controller SHIPPED**; iter 6 (fo-6-b918dbb8, 1 GPU worker, 0 conflicts) shipped GV3 (agent.local #21 opt-in `-AutoRamp` -- monotonic model-affine epochs M0->M1->S0, a pre-frozen `lifeorch.goal_verification/0.1` success contract, whole-task gpu lease + exact warm-server residency-key matching, hard/2-strike triggers, duplicate-side-effect guard, governor trace; commit 0005e41, module 21 ONLY). CALIBRATION: resident 9B DIRECT decisions 6/6 vs 3B 4/6 (S0 holds; S0 needs >=~1024 tok or the 9B returns empty -- a HARD trigger). GATES: off-machine 50/50 + agent.local 87/87 green; LIVE floor->M0 no-escalation + engineered ramp M0->M1->S0 (1 swap), 0 orphans. The shipped floor default is unchanged (`-AutoRamp` is off by default). Next = iteration 7 (Phase 3 Stage-2 / wire -AutoRamp into a caller / residual live-GUI Console pass). **[prior active D-0058]** **FAN-OUT loop through ITERATION 5 (D-0058) -- Console teardown orphan-sweep + Module 30 packet-input validation shipped; Governor Phase 3 frontier second opinion received (BUILD = iteration 6)**; iter 5 (fo-5-c76df95a, 2/2, 0 conflicts) shipped TD (widgets/03 run-teardown reaps the detached warm llama-server via a scope-guarded PID-set diff + a -Live 0-orphan assertion, 033fd6f, cloud 101/101 + live 112/112) + MK (orchestrate.fanout #30 handoff packet-input contract validation with input_warning surfacing, 2afd5de, live 71/71; plan/report/status + plan.json unchanged). Phase 3 is now DESIGNED + frontier-refined (monotonic model-affine epochs M0->M1->S0->X0, a pre-frozen lifeorch.goal_verification/0.1 success contract, whole-task gpu lease, exact resident-model manifest -- claude/ADAPTIVE_RESOURCE_GOVERNOR.md). Next = iteration 6 (build the Phase 3 Stage-1 slice). **[prior active D-0057]** **FAN-OUT loop through ITERATION 4 (D-0057) -- Governor Phase 2 DONE**; iter 4 (fo-4, 3/3) shipped E2 (DETACHED warm/persistent llama-server in model.gateway #7, f8c961a -- warm reuse ~1ms vs ~1200ms cold, 0 orphans; the wedge fix vindicated), H (Verification Console audit loop validated end-to-end + caught a real stale-packet-input defect, 174360d), I (frontier.bridge return-capture hardened + a real Phase 3 escalation pack, b17a945). res.lease trio complete; executor+watchdog hardened+restarted-live; audit loop exercised. Next = iteration 5 (Governor Phase 3 / the Module 30 packet-input fix / the Console teardown fix). Fan-out orchestrator handoff: core-docs/FANOUT_ORCHESTRATOR_HANDOFF.md. **[prior active D-0056] FAN-OUT loop through ITERATION 3 (D-0056)** -- iter 2 shipped D (res.lease doc:<path> -> doc.io #20, d2a7352, 106/106) + F (Module 30 prompt-template fix, 581f854, 56/56) but E (warm-server) FAILED and its orphaned llama-server WEDGED the executor (recovered by killing it out-of-band); iter 3 (single worker G) SHIPPED executor+watchdog hardening (e5b93ab, tests 16/16 + 33/33) -- reap detached children before finalize, resilient poll loop, watchdog wedge-detection + a TZ parse-bug fix; executor restarted onto it (instance 30aa2f29, health fields live), watchdog running. res.lease consumer trio COMPLETE (gpu/git/doc). Next = iteration 4 (rerun E as a DETACHED warm server + 2 companions). **[prior active D-0055] FAN-OUT DOGFOOD (iteration 1) COMPLETE (D-0055, plan fo-1-20ed8a0b)** -- orchestrate.fanout #30 drove 3 workers to done (res.lease gpu->model.gateway #7 0c6d5c9 42/42; res.lease git->dev.ship 5530418 39/39; frontier.bridge #31 built f52f21d 65/65); 0 conflicts; next = iteration 2 (doc: consumer / Governor Phase 2). **[prior active] the FAN-OUT ORCHESTRATOR -- `orchestrate.fanout` (Module 30) SHIPPED (D-0054, commit `2ffe162e`; 51/51 cloud + 51/51 live via the job-runner)** -- the deterministic scaffolding for N parallel worker Cowork sessions, built on `res.lease` #29: `plan` schedules (dispatch_now = trial of MaxParallel, clamped to one GPU worker; gpu-serialization + doc-ownership conflict flags) and emits each worker's ordered gpu->git->doc res.lease acquire/release commands + a prompt + one Nicholas check-in prompt (+ an optional read-only res.lease preflight); `report`/`status` roll up worker progress + a ready-for-handoff cadence; `handoff` emits a Verification Console packet (`lifeorch.verification.packet/0.1`) + next-iteration prompts. Orchestrator/non-producer; ships `FANOUT_PROTOCOL.md`. Next unit = a fan-out dogfood (a real 2-worker plan Nicholas runs) or wiring res.lease consumers. **[prior]** the resource-arbitration lock/lease layer -- `res.lease` (Module 29) SHIPPED (D-0053, commit 36d7e0be; 38/38 cloud + 41/41 live via the job-runner)** -- the multi-instance coordination primitive: an atomic filesystem lease with a TTL (a `gpu` lease + a `git` commit lock + `doc:<path>` ownership), race-safe stale reclaim, holder identity + a lease_id token, blocking/non-blocking acquire; deterministic, parallel_safe:true, not a review producer; unblocks the fan-out orchestrator (next unit). **[prior]** **Widget #3 -- the Verification Console SHIPPED (D-0051, commit f7e7b289; 73/73 cloud + 80/80 live)** -- the human-AUDIT surface for the offload / verify-cost spine; Claude writes a verification packet and Nicholas runs + checks it locally through Invoke-Skill.ps1, exporting a verification-result JSON Claude reads back. Recent history: **Module 0 job-runner SHIPPED (D-0048): `Invoke-DevShip.ps1` + `exec-job.sh`** -- one executor job gate+commits a unit (sha + AST + tests, fail-closed) and returns a compact JSON; dogfood-committed by dev.ship itself (`5644b9ba`), 27/27 + 24/24. **Widget #2 (Module Launcher / Registry Browser) SHIPPED 2026-07-27 (D-0049, commit `a699ac6`; 62/62 cloud + 71/71 `-Live` via the job-runner; **post-ship UI fix `c509e571` -- registry double-wrap + splitter layout, 64/64 + 75/75**). Next unit = Phase B Widget #3 -- the Verification Console (D-0050)** (`MODULE_ROADMAP.md -> Build priority`). **Modules 0–25 complete** (0 executor · 1 `skill.bootstrap` · 2
...
- **DIRECTION (2026-07-27, D-0050):** past MVP the project drives ONE spine -- the OFFLOAD / AUDIT LOOP under the verify-cost rule (Claude offloads only what is cheaper to VERIFY than to do; deterministic modules = Claude's hands; model modules only where machine- or human-checkable). Active unit = **the fan-out orchestrator (`orchestrate.fanout` #30) -- SHIPPED (D-0054, commit `2ffe162e`; built on res.lease #29)**; **iterations 1-3 of the fan-out loop are done (D-0056). Iter 1: gpu->#7 + git->dev.ship + frontier.bridge #31. Iter 2: doc:<path>->doc.io (d2a7352, 106/106) + Module 30 template fix (581f854) shipped; E (warm-server) failed + wedged the executor -> recovered. Iter 3: executor+watchdog hardening (G, e5b93ab, 16/16 + 33/33) shipped + restarted-live. res.lease consumer trio complete.** Iteration 4 (fo-4) is done (D-0057, 3/3): E2 shipped the DETACHED warm server (Governor Phase 2 DONE, f8c961a), H validated the Verification Console audit loop end-to-end (174360d), I hardened frontier.bridge + produced a real Phase 3 escalation pack (b17a945). Iteration 5 is DONE (D-0058, fo-5-c76df95a, 2/2): TD shipped the widgets/03 teardown orphan-sweep (033fd6f) + MK shipped the Module 30 packet-input validation (2afd5de); the Governor Phase 3 frontier second opinion was couriered back and folded into the design. Next = iteration 6 (build the Governor Phase 3 Stage-1 slice per the refined model-affine-epoch design). See core-docs/FANOUT_ORCHESTRATOR_HANDOFF.md. Cadence = housekeeping -> implement one unit -> housekeeping -> handoff, run HOT. Multi-instance buildout is a live direction (needs a GPU-lease / commit-lock / doc-ownership layer first). See `DECISION_LOG.md` D-0050.

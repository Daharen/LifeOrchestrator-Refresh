...
| #37 retrieval.eval | **eval 0.6.0 (i34): a SEPARATE hierarchy_eval.py + `-Op hierarchy-eval`, ADDITIVE (retrieval_eval.py + lib/* unchanged, benchmark byte-identical); navigation-cost sub-linear + DUAL recall (fast-beam/guaranteed/end-to-end packet) + 5/5 adversarial + 11/11 Tier-1 gates; off-machine 26/26; the ~200MB real-corpus rehearsal scaffolded+FLAGGED OPEN (Tier-1 acceptance gate, tier1_accepted=false on synthetic)** | `ad20fa6` | 08-05 |
...
| #40 context.compiler | **0.6.0 (i34): shortlist-and-descend PLAN + safe-pruning enforcement + retrieval_completeness + hierarchy packet-identity + navigation-vs-evidence closure; consumes #36 via an injected port; imports #37 READ-ONLY; off-machine 322/322 + on-device 45/45; flat compile byte-identical to 0.5** | `3968c96` | 08-05 |
...
**i34 CLOSED (D-0098 contract + D-0099 close; HEAD `ad20fa6`, clean tree).** Both lanes shipped: the Tier-1 bounded-fanout HIERARCHY nav layer (#36 artifact.search 0.5.0 `356ab64` + #40 context.compile 0.6.0 `3968c96` + #37 retrieval.eval eval 0.6.0 `ad20fa6`) AND the per-task WORKING-MEMORY store (NEW #42 working.memory 0.1.0 `601a2db`, realizes MEMORY_CONTRACT A5 U3'). The D-0077 hierarchy fold smoke PASSED 38/38. Tier-1 ACCEPTANCE is NOT claimed on synthetic-only (#37 tier1_accepted=false); the ~200MB real foreign-corpus REHEARSAL (scaffolded + FLAGGED by #37) is the pre-freeze/pre-activation gate.

**NEXT = i35 CONSUMER WIRING (the glue the folds surfaced -- built producers, unwired public end-to-end):** (1) ship the real hierarchy_port into #40's `-Retriever artifact_search` CLI (today FLAT-only -- SEAM 1 leaf hydration + SEAM 2 prune-certificate composition); (2) wire #40's packet working_memory region to #42's get_active_head (today "reserved; empty"); (3) the multi-channel query ROUTER (query_class is a stub); (4) the ~200MB real-corpus rehearsal (Tier-1 acceptance gate). Plus the standing **P0-1 adversarial-injection suite** before any action-capable release. Candidate menu + work orders: **`FANOUT_ORCHESTRATOR_HANDOFF.md`** section 4.
...
**Last updated:** 2026-08-05 -- i34 CLOSED (D-0098 contract + D-0099 close; HEAD `ad20fa6`, clean tree): both lanes shipped -- the Tier-1 bounded-fanout HIERARCHY nav layer (#36 0.5.0 `356ab64` + #40 0.6.0 `3968c96` + #37 eval 0.6.0 `ad20fa6`) + the per-task WORKING-MEMORY store (NEW #42 0.1.0 `601a2db`, realizes A5 U3'); the D-0077 hierarchy fold smoke PASSED 38/38. Tier-1 ACCEPTANCE gated on the ~200MB real-corpus rehearsal (#37 tier1_accepted=false on synthetic). NEXT = i35 consumer wiring + query router. (Doc debt: hot docs still over budget -- PB-3 slim remains a named unit.)

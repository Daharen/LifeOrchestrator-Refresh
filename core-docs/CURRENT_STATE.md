...
| #37 retrieval.eval | **0.3.0 (i30): NEW lib/selpol_rrf_v1.py selection-policy library (P1-1, CONTEXT_PACKET_CONTRACT s4; owned here, consumed by #40) + eval-0.3 per-stage/packet_disposition; rerank() now a thin wrapper (byte-identical A/B); 27 library-unit + benchmark + cross-env tests -Live green | `99bb627` | 08-03 |
...
| #40 context.compiler | **0.2.0 (i30): context_packet/0.2** -- P0-1 control_plane/task_input/evidence separation (non_execution gate + injection test) + P0-3 disposition + P0-4 consumer_profile/transport_accounting (count_is_exact=false) + P1-1 selpol seam (selpol_reference) + P1-5 identity/lineage + A2 provenance modes; 148/148 python + -Live real slice | `f06e6e7` | 08-03 |
...
- **The selpol producer/consumer divergence is OPEN (i30 D-0088):** the P1-1 'one selection owner' is NOT yet realized -- #40 context.compiler ships against its in-module `selpol_reference.py`, which selects DIFFERENTLY from #37's canonical `lib/selpol_rrf_v1.py` (different relevance-primary [rank-RRF vs raw fused_score], weight scales, diversity model, output fields; a mechanical `AUTHORITY_POINTS` break). NEITHER is a defect -- CONTEXT_PACKET_CONTRACT s4 froze the interface + stages but not the weights/relevance-primary/diversity/output. Resolve at **i31 selection-policy settle**: pin s4 + re-ship #40 to import #37's canonical (retire the reference); re-smoke for byte-identical selection.
...
**i30 (D-0087 + D-0088) SHIPPED CONTRACT-HARDENING: the context_packet/0.2 contract (NEW `core-docs/CONTEXT_PACKET_CONTRACT.md` + MEMORY_CONTRACT A2/A3) + the 3-lane conformance wave (plan fo-30-dd453156): context.compiler #40 0.2.0 (`f06e6e7`) + retrieval.eval #37 0.3.0 (`99bb627`; NEW lib/selpol_rrf_v1.py) + skill.card #41 0.2.0 (`54c2e79`). The D-0077 fold PASSED the #41->#36->#40 chain but CAUGHT a selpol producer/consumer divergence (#40 reference vs #37 canonical). NEXT = i31 SELECTION-POLICY SETTLE (pin s4 + re-ship #40 to import #37's canonical), then the deferred P1-2..P1-9 + P2 + the P0-1 adversarial injection suite.** Resume: the wave is DONE + committed (HEAD chain 6d9da94 slots -> 1eafd2c #41 -> dc293ef #37 -> b89eda0 #40; slots archived + reset). i30 scope = fold the frontier P0-1..P0-5 + P1-1 (digest `research/2026-08-02-frontier-wave3-design-redteam.md`) into a MEMORY_CONTRACT/context_packet 0.2 amendment (control/evidence separation [P0-1, safety-critical], provenance hash split [P0-2], packet_disposition [P0-3], consumer/tokenizer profile [P0-4]) + module revisions (#40 context_packet 0.1->0.2; #41 skill->summary activation card [P0-5]; one selection-policy lib shared by #40/#37 [P1-1]); P0-1 is a HARD GATE before ANY side-effecting integration. Shape (orchestrator-only amendment like i26/i28 vs a small worker wave) TBD at i30 scoping. Work orders: **`FANOUT_ORCHESTRATOR_HANDOFF.md`** section 4.
...
**Last updated:** 2026-08-03 -- i30 (D-0087 + D-0088): CONTRACT-HARDENING shipped -- the context_packet/0.2 contract (NEW `CONTEXT_PACKET_CONTRACT.md` + MEMORY_CONTRACT A2/A3, D-0087) + the 3-lane conformance wave (fo-30-dd453156, D-0088): #40 context.compiler 0.2.0 (`f06e6e7`) + #37 retrieval.eval 0.3.0 (`99bb627`; NEW lib/selpol_rrf_v1.py) + #41 skill.card 0.2.0 (`54c2e79`; A3 skill->summary). The D-0077 fold PASSED the #41->#36->#40 chain; it CAUGHT a selpol producer/consumer divergence (#40 reference vs #37 canonical select differently) -> NEXT = i31 selection-policy settle. (Doc debt: hot docs still over budget -- a slim pass remains a named unit.)

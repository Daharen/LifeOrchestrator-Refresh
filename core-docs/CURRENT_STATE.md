...
| #37 retrieval.eval | **0.2.0 (i29): 119/119 cloud + 119/119 `-Live`** cross-env byte-identical; MEMORY_CONTRACT s6 eval-0.2 (evidence-group labels + temporal intent + negatives/no-answer + hybrid attribution [vector EMPTY] + provenance VALIDATION + nDCG/precision@K/forbidden/stale/dup) + a DETERMINISTIC reranker (MEASURED A/B vs raw order); 0.1 regression-green (was 69/69) | `dc293ef` | 08-02 |
...
| #40 context.compiler | 0.1.0 (i29): 46/46 python + 16/16 pwsh cloud + 23/23 `-Live`; NEW deterministic `lifeorch.context_packet/0.1` compiler (normalize -> #36 retriever-0.2 seam -> rerank/diversity -> token budget -> packet + expand seam + eval hooks); real-slice spans present + provenance reproduced; byte-identical re-run; 0 orphans | `b89eda0` | 08-02 |
| #41 skill.card | 0.1.0 (i29): 72/72 python cloud + 80/80 `-Live`; deterministic section-9 skill-card generator + skill index (`sklcard_` s1 records, drop-in for #36 ingest) + Stage-1 eligibility + Stage-2 lexical seam; #38 boundary; byte-identical; 0 orphans | `1eafd2c` | 08-02 |
...
**i29 (D-0086) SHIPPED WAVE 3 (context compiler + skill retrieval; plan fo-29-87dbfa0b): context.compiler #40 (`b89eda0`) + retrieval.eval #37 0.2.0 (`dc293ef`) + skill.card #41 (`1eafd2c`). The D-0077 fold smoke PASSED on real data; the frontier design red-team (pack d57fead3) FOLDED -- GO for the read-only build, NO-GO for freezing the new contracts, NO-GO for side-effecting until the safety-critical P0-1. NEXT = i30 CONTRACT-HARDENING.** Resume: the wave is DONE + committed (HEAD chain 6d9da94 slots -> 1eafd2c #41 -> dc293ef #37 -> b89eda0 #40; slots archived + reset). i30 scope = fold the frontier P0-1..P0-5 + P1-1 (digest `research/2026-08-02-frontier-wave3-design-redteam.md`) into a MEMORY_CONTRACT/context_packet 0.2 amendment (control/evidence separation [P0-1, safety-critical], provenance hash split [P0-2], packet_disposition [P0-3], consumer/tokenizer profile [P0-4]) + module revisions (#40 context_packet 0.1->0.2; #41 skill->summary activation card [P0-5]; one selection-policy lib shared by #40/#37 [P1-1]); P0-1 is a HARD GATE before ANY side-effecting integration. Shape (orchestrator-only amendment like i26/i28 vs a small worker wave) TBD at i30 scoping. Work orders: **`FANOUT_ORCHESTRATOR_HANDOFF.md`** section 4.
...
**Last updated:** 2026-08-02 -- i29 (D-0086): WAVE 3 SHIPPED (context compiler + skill retrieval; plan fo-29-87dbfa0b) -- context.compiler #40 0.1.0 (`b89eda0`; `lifeorch.context_packet/0.1`) + retrieval.eval #37 0.1.0->0.2.0 (`dc293ef`; eval-0.2 + a deterministic reranker) + skill.card #41 0.1.0 (`1eafd2c`; section-9 cards + skill index + eligibility/retrieval seam). The D-0077 fold smoke PASSED on real data; the frontier design red-team (pack d57fead3) FOLDED -- GO for the read-only build, NO-GO for freezing the new contracts, NO-GO for side-effecting use until the safety-critical P0-1 (control-plane-vs-evidence separation). NEXT = i30 CONTRACT-HARDENING (P0-1..P0-5 + P1-1). (Doc debt: hot docs still over budget -- a slim pass remains a named unit.)

...
- **DIRECTION (2026-07-27, D-0050):** past MVP the project drives ONE spine -- the OFFLOAD / AUDIT LOOP under the verify-cost rule (Claude offloads only what is cheaper to VERIFY than to do; deterministic modules = Claude's hands; model modules only where machine- or human-checkable). Active unit = **the fan-out orchestrator (`orchestrate.fanout` #30) -- SHIPPED (D-0054, commit `2ffe162e`; built on res.lease #29)**; **iterations 1-3 of the fan-out loop are done (D-0056). Iter 1: gpu->#7 + git->dev.ship + frontier.bridge #31. Iter 2: doc:<path>->doc.io (d2a7352, 106/106) + Module 30 template fix (581f854) shipped; E (warm-server) failed + wedged the executor -> recovered. Iter 3: executor+watchdog hardening (G, e5b93ab, 16/16 + 33/33) shipped + restarted-live. res.lease consumer trio complete.** Next = iteration 4: rerun E as a DETACHED warm/persistent llama-server (task returns; server outlives the job) + 2 companion units. Cadence = housekeeping -> implement one unit -> housekeeping -> handoff, run HOT. Multi-instance buildout is a live direction (needs a GPU-lease / commit-lock / doc-ownership layer first). See `DECISION_LOG.md` D-0050.

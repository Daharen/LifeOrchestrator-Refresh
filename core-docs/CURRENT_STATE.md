...
**NEXT = i36 (the deferred consumer glue + the Tier-1 flip + the P0-1 pre-work):** (1) wire #40's packet working_memory region to #42's get_active_head (today "reserved; empty"); (2) the multi-channel query ROUTER (query_class is a stub); (3) the FULL ~200MB rehearsal against #40's WIRED descend path -> flip project `tier1_accepted` iff it passes; (4) a #36 `get-record` by-rvid op (Lane A's fold reconciliation); (5) FREEZE the action-authorization contract (proposal/tool_manifest/action_permit/completion) THEN build the P0-1 adversarial-injection SUITE (design in hand). Nothing is action-capable (`non_execution:true` holds). Candidate menu + work orders: **`FANOUT_ORCHESTRATOR_HANDOFF.md`** section 4. **Audit-surface program adopted (D-0101):** R-1 stage-trace EMISSION binds the router (born instrumented; asserted by the D-0077 fold); Widget 05 "Provenance Map" is a NON-DISPLACING i36 coding-lane candidate (Widget 06 i37-i38; Phase 2 possession post-i40 red-team-gated); the staged target is `research/2026-08-05-audit-pipeline-target.md` (cadence PB-4, review_due i39).

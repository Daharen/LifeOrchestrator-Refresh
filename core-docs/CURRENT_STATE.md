...
- **Active module:** **Module 0 expansion -- executor job-runner + `dev.ship` unit harness (D-0047, IN PROGRESS)** -- collapse the per-unit ship/verify/test/commit/mirror ceremony to cut frontier token overhead; then resume `MODULE_ROADMAP.md -> Build priority` (Phase B Widget #2). **Modules 0–25 complete** (0 executor · 1 `skill.bootstrap` · 2

...
**i28 (D-0085) SETTLED the memory contract (MEMORY_CONTRACT Amendment A1, v0.1.1) + conformed episode.record #39 -> 0.1.1; the D-0077 re-smoke PASSED 30/30 (0 bridging, 0 rejections). NEXT = Wave 3 (context compiler + skill-card/registry + retrieval reranker).**
...
**Last updated:** 2026-08-02 -- i28 (D-0085): CONTRACT-SETTLE wave -- MEMORY_CONTRACT Amendment A1 (record-envelope v0.1 -> v0.1.1: envelope status = a single s5 STRING; `record_kind` enum CLOSED, `episode_stage` retired) + episode.record #39 -> 0.1.1 conformance (status -> the s5 string; per-stage detail folded into `episode.body.stage_sequence` + `has_stage` edges; suite 114->123). The D-0077 re-smoke PASSED 30/30 (would_bridge 0, 0 rejections) -> the two i27 divergences RESOLVED. NEXT = Wave 3 (context compiler + skill-card/registry + retrieval reranker). (Doc debt: hot docs still over budget -- a full slim pass remains a named unit.)

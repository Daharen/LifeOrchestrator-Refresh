...
- `last_reviewed: i58` (D-0153 -- the i58 cadence check ran at close: review_due reached, no tier-prereq flip since i45 (A3 remains designable, prereqs met), next_increment unchanged; NON-DISPLACING -- the i58 universal-front-door freeze outranked the audit increment, D-0152)
- `review_due: i60` (bumped +2 at i58 -- s5 no-spare-lane bump: the i58 wave is the design/freeze + red-team of the universal derived-front-door contract (no coding lane), so no audit coding lane was spare; the w08 explain-window-close defect (D-0134) still rides ANY earlier w08 touch)

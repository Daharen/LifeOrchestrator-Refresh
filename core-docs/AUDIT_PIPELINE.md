...
- `next_increment: LRAP phenomenological completion set (D-0125): v1 is action+flag-legible but NOT phenomenological even on the built cases (Nicholas cannot adopt the agent's role). The gate needs, together: (0) POSSESSION -- surface EXACTLY what the agent saw (incl. any currently-opaque context) + the per-step RATIONALE/logic, so the human makes the COMPARATIVE judgment WITHOUT the widget substituting a semantic verdict (resolves the F1-vs-P9 tension); (1) the raw-prompt FRONT step (initial input to judge against; step-1 INPUT P2 -> upstream emission); (2) the LIVE ride-along (audit-tag launch + per-step pause/unpause; A2.2); (3) the OUTPUT side + instruction<->output reconciliation. Each design-first -> red-team-gated; 05/06/07/08 stay the descend/replay base`

...
| PB-5 | #37 retrieval.eval fold-reconciliation + version hygiene (D-0108): (a) re-pin tests/test_rehearsal_eval.py WIRED_STRUCTURAL_DIGEST -> `sha256:d0d54aba...` (re-derived + verified after #36 0.7.0's fast-beam moved the wired descend structure; the wired metric IMPROVED -- 11/11 criteria, hpr 58823->117647, guaranteed/packet 1e6 -- only the fingerprint moved); (b) reconcile a PRE-EXISTING #37 -Live `manifest version` inconsistency -- skill.json is 0.8.0 but the harness check + the worker envelope emit 0.7.0 (undetected since i37/i38 never re-ran #37 -Live). ONE small #37 lane (worker+harness+manifest -> a single version source of truth + the wired re-pin) -> #37 -Live fully green. The orchestrator did NOT blind-fix the version semantics; the wave working tree was reverted clean (nothing half-committed). | Next #37 touch OR i40. | D-0108 |

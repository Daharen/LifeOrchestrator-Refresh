...
> composition, real-time perception (27-49), broad training. **Wave 1 SHIPPED i25 (D-0082): #35 embedding.local + #36 artifact.search + #37 retrieval.eval; D-0077 smoke PASSED; memory red-team GO -> the retrieval-record/provenance contract freeze (i26, D-0083, MEMORY_CONTRACT.md). **Wave 2 (memory RECORDS) SHIPPED i27 (D-0084):** #36 artifact.search 0.2.0 (record-envelope + ingest_records SINK + retriever-0.2 + float32 BLOB vectors + catalog hardening) + NEW #38 repo.intel (typed-record producer) + NEW #39 episode.record (episode+failure schema + recorder); the D-0077 fold PASSED (2 divergences bridged). **Wave-2 divergences RESOLVED i28 (D-0085): episode.record #39 -> 0.1.1 + MEMORY_CONTRACT Amendment A1 (v0.1.1); the re-smoke PASSED 0-bridging.** **Wave 3 (context compiler + skill retrieval) SHIPPED i29 (D-0086):** NEW #40 context.compiler `context.compile` 0.1.0 (`b89eda0`; `lifeorch.context_packet/0.1`) + #37 retrieval.eval 0.1.0->0.2.0 (`dc293ef`; s6 eval-0.2 + a deterministic reranker) + NEW #41 skill.card 0.1.0 (`1eafd2c`; section-9 cards + `sklcard_` s1 skill index + Stage-1 eligibility + Stage-2 lexical seam); the D-0077 fold PASSED on real data; the frontier design red-team (d57fead3) FOLDED -- GO for the read-only build, NO-GO for freezing the new contracts. **i30 CONTRACT-HARDENING SHIPPED (D-0087/D-0088, plan fo-30-dd453156):** the context_packet/0.2 contract (NEW `CONTEXT_PACKET_CONTRACT.md` + MEMORY_CONTRACT A2/A3) + #40 context.compiler 0.2.0 (`f06e6e7`; three-region control/evidence packet + disposition + consumer profile + selpol seam) + #37 retrieval.eval 0.3.0 (`99bb627`; the `selpol_rrf_v1` selection-policy library) + #41 skill.card 0.2.0 (`54c2e79`; record_kind skill->summary). The D-0077 fold PASSED the #41->#36->#40 chain but CAUGHT a selpol producer/consumer divergence -> **NEXT = i31 SELECTION-POLICY SETTLE** (pin s4 scoring + re-ship #40 to import #37's canonical). digest `research/2026-08-02-frontier-wave3-design-redteam.md`. Original scope: embedding adapter + `artifact.search`

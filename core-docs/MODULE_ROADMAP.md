...
> composition, real-time perception (27-49), broad training. **Wave 1 SHIPPED i25 (D-0082): #35 embedding.local + #36 artifact.search + #37 retrieval.eval; D-0077 smoke PASSED; memory red-team GO -> the retrieval-record/provenance contract freeze (i26, D-0083, MEMORY_CONTRACT.md). **Wave 2 (memory RECORDS) SHIPPED i27 (D-0084):** #36 artifact.search 0.2.0 (record-envelope + ingest_records SINK + retriever-0.2 + float32 BLOB vectors + catalog hardening) + NEW #38 repo.intel (typed-record producer) + NEW #39 episode.record (episode+failure schema + recorder); the D-0077 fold PASSED (2 divergences bridged). **Wave-2 divergences RESOLVED i28 (D-0085): episode.record #39 -> 0.1.1 (status = the s5 string; `episode_stage` retired -> stages in `episode.body.stage_sequence`) + MEMORY_CONTRACT Amendment A1 (v0.1.1); the re-smoke PASSED 0-bridging. NEXT = Wave 3 (context compiler + skill-card/registry + retrieval reranker).** Original scope: embedding adapter + `artifact.search`

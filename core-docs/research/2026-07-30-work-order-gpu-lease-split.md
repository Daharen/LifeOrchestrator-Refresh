...
## Frontier review folded (i18, 2026-07-30 — pack `42ad8308`; full answer: `research/2026-07-30-frontier-review-self-tasking-orchestration.md`)

An off-box GPT-5.x review of this work order + the trajectory review. **Verdict: the lease split is the right keystone and the R1a-primitive / R1b-consumer split is SOUND** — provided R1a is treated as PROVISIONAL (protocol implemented, hardware behavior unproven). The following supersede / tighten this work order for **R1b**:

- **GOVERNANCE (applied at i18 close-out):** R1a must **NOT** close findings 1/13/14 or declare "only soak remains" — those close **only after R1b**. The State-updates section above is amended: R1a shipped the primitive; findings 1/13/14 stay OPEN pending R1b + the live-GPU proof.
- **Separate THREE identities**, not one overloaded fencing token: `gpu_authority_epoch` (increments when exclusive GPU authority changes), `resident_generation` (increments when a server is started/replaced/invalidated), `exec_lease_id` (one execution acquisition). Every inference + publication carries `owner_id + gpu_authority_epoch + resident_generation + exec_lease_id`; side effects **assert** the epoch rather than advancing it. (R1a shipped a single monotonic `fencing_token`; R1b should split it.)
- **The hand-off is ONE scheduler-owned atomic transition** (reserve → fence old owner off new exec → drain/cancel active exec → invalidate its publish authority → shut down its resident → confirm tree gone → confirm stable headroom → grant → start+health-check → publish new generation). No interval where the old owner can reacquire execution, and none where the new owner holds an ordinary lease while eviction is merely "in progress."
- **Explicit in-flight revocation:** a pin revokes immediately, but an ACTIVE execution gets a bounded **drain → cancel → supervisor Job-Object tree-kill**; any result arriving after fencing is **DISCARDED** even if the model call technically succeeded.
- **Do not call the evictor / supervisor / telemetry under the lease-state mutex** — record a transition intent + txn id under the mutex, RELEASE it, do the external work, then reacquire and commit only if the txn's epoch is still current (a lock-order / re-entry hazard even with the public gpu→git→doc order correct).
- **WDDM / 2080 Ti headroom:** the prepared-GPU guarantee only covers cooperating MANAGED workloads. Confirm the managed tree exited; poll headroom **stable across ~3 observations ~250 ms apart** after tree-exit; recheck immediately before load; treat an allocation failure as a normal failed prepare; NEVER kill an unidentified VRAM consumer. `required_vram` = measured PEAK (weights + KV + compute buffers + overhead + margin), initially max(≈512 MiB, 10 %), later per-`config_key` high-water marks — NOT GGUF file size.
- **Executor re-wedge protection (D-0055/56 class):** only the durable supervisor may start/terminate model servers; the requester never kills a PID; the whole tree must enter the Job Object before it can be published; startup = spawn → job-assign → start → health-check → fenced publish; shutdown = graceful → bounded wait → Job-Object terminate → tree-exit confirm → VRAM confirm; reconcile a crash in PREPARING/DRAINING/STARTING from the transaction journal.
- **The R1a mock evictor must become ADVERSARIAL** (simulate: late eviction; a stale idle callback after a new grant; partial tree termination; headroom never reached; headroom reached then fell; cancellation during prepare; lease expiry during exec; a result arriving after revocation; a crash after start before publish; equal-priority simultaneous acquisition). R1b live tests must add: **revoke while the old owner has an ACTIVE inference** (not merely an idle pin), and **a deliberately late old-generation result proven unable to publish / alter the manifest / kill the new server**. Only after those pass do findings 1/13/14 close.
- **Sequencing:** default-ON is NOT required for an opt-in orchestrator proof. R1a → R1b → a minimal opt-in **ABA** baton proof → soak → R2 (default-ON) → expanded #26. Strong preflight (R3) alongside R1b/proof but NOT in the same release wave. The soak remains the real default-ON gate (≥200 transitions, ≥25 forced revocations, ≥25 injected crash/expiry/cancel, long idle/resume, zero stale publications, zero grants-before-confirmed-headroom, zero orphaned managed servers, zero unreconciled transitions; record p50/p95 handoff + headroom-confirm latency).

...
- **i58 CLOSED (D-0153): the universal derived-front-door contract FROZEN + 3-adversary red-teamed (7 breaks, 2 ratification-blocking -> hardened s8 authoritative).** Design/freeze wave, no module code. Frozen set: `research/2026-08-15-i58-universal-front-door-contract.md` + `-front-door-redteam.md` + `-front-door-hardened.md`. NEXT = i59 = the root migration (D-0152, AMENDED by D-0153 s8 rule 1): START_HERE -> a ~0.5-1 KB bootstrap kernel AND a generator-INDEPENDENT cold-boot card (the kernel alone is insufficient -- CB1) + a uniform rebuild interface; the PCB packet = the first fully-owned generated front door. AUDIT review_due i60 NON-DISPLACING. Seat = Fable 5 until Nicholas declares settled (D-0134).
...
- **i55-i57 (D-0149/D-0150/D-0151): the PB-6 decision re-layer -- DESIGNED -> BUILT -> BOOT-WIRED (PB-6 COMPLETE).** i55 designed PB-7 + shipped the retrieval-byte monitor (`99cb32b`); i56 built #45 producer + #40 verb 0.10.0 (D-0077 fold smoke fixed 2 seam breaks, `206bdd39`); i57 wired it to BOOT (`d16cdc0`), boot stops whole-ingesting the index.
- **i58 (D-0153): the universal derived-front-door contract FROZEN + 3-adversary red-teamed** -- 7 breaks (CB1 kernel-recovery + CB4 cross-front-door-currency ratification-blocking) -> hardened s8 AUTHORITATIVE; i59 amended (START_HERE kernel + a generator-independent cold-boot card). Design/freeze wave, no module code; 3 digests `research/2026-08-15-i58-*.md`; AUDIT review_due i58->i60.

Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` * artifacts `.../runtime/artifacts/<id>/` * leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 59.**

## 4. Current frontier -- i58 CLOSED (D-0153): the universal derived-front-door contract FROZEN + red-teamed; NEXT = i59 = the root migration (START_HERE kernel + a generator-independent cold-boot card; D-0152/D-0153); AUDIT review_due i60; SP3/M-03 open

**i58 CLOSED (D-0153).** The universal derived-front-door contract is FROZEN + 3-adversary red-teamed (orchestrator-inline, D-0119). Frozen set: `research/2026-08-15-i58-universal-front-door-contract.md` (the law + 5 front-door properties + the two-engine seam + spill-not-compress + G1-G6) + `-front-door-redteam.md` (7 confirmed breaks) + `-front-door-hardened.md` (the AUTHORITATIVE s8 invariant set). 3 in-session cloud-subagent adversaries (fronts: universality/judgment, spill/boundedness, currency/composition) confirmed 7 breaks -- **CB1 (the kernel is the wrong+insufficient recovery survivor) + CB4 (currency does not compose across front doors) were ratification-blocking** -- so the contract freezes WITH the hardened s8 authoritative (the two-engine architecture + enumerable-interior re-layer HELD; the naive universal-boundedness claim did not). **NEXT = i59 = the root migration, AMENDED by s8 rule 1:** START_HERE -> a stable ~0.5-1 KB bootstrap kernel AND a generator-INDEPENDENT cold-boot card (the kernel alone is insufficient -- a broken generator leaves it pointing at down machinery) + register a uniform rebuild interface; the PCB packet = the first fully-owned generated front door. **i60+** = class-by-class (CURRENT_STATE interior, handoff ledger, catalogs, backlogs, gotchas, tests, research), each design-first -> red-team-gated, carrying the s8 build requirements (pin-the-unrecoverable, task-match keys, classifier oracle, frozen-replayed synopsis, compile epoch, two-phase close); the renderer slim-ladder is removed during migration. AUDIT review_due i58 DISCHARGED (no tier-prereq flip; review_due->i60; NON-DISPLACING). SP3/M-03 OPEN; P0-1 activation FROZEN; SEALED_CHECK_47 retained; #40 beam-width / PB-2 / w08 deferred (D-0134).
...
**i58: the universal derived-front-door contract FROZEN + 3-adversary red-teamed (orchestrator-inline, Opus 4.8 seat) -- D-0153.** Design/freeze wave (no module code, no GPU): 3 research digests frozen (contract + redteam + hardened s8 AUTHORITATIVE); 7 breaks confirmed (CB1 kernel-recovery + CB4 cross-front-door-currency ratification-blocking). The N7 close-refold restamped the map at HEAD (0 stale on boot_read); the overlay frontier redirected to i59; both monitors regenerated; core-docs re-mirrored; GitHub mirror re-pushed. No held res.lease; heartbeat degraded:false; 0 UNMANAGED orphans; doc-gate PASS on every commit. **Open Nicholas items: i59 = the root migration (START_HERE kernel + a generator-independent cold-boot card, per D-0153 s8 rule 1); license M-03 (SP3) when ready.** The w08 window-close defect stays routed to the next w08 touch (D-0134).

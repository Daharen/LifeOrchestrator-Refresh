...
- **i59 CLOSED (D-0154): the ROOT front-door migration SHIPPED.** START_HERE -> a stable ~1 KB hand-maintained boot KERNEL (Mode A packet / rebuild-once / Mode B card; NO changing state) + NEW `core-docs/COLD_BOOT_CARD.md` (generator-INDEPENDENT recovery survivor) + NEW `ops/frontdoor/` uniform rebuild interface (registry + `Rebuild-FrontDoors.ps1` driver + `frontdoor-gate.py` close-step) + the migration gate. 3-adversary red-teamed (D-0119; architecture HELD, 13 gaps folded incl. fail-closed-by-DETECTION close). Frozen: `research/2026-08-15-i59-root-migration-{hardened,redteam}.md`. NEXT = i60 = the first CLASS migration (D-0153 s8); class-by-class thereafter. AUDIT review_due i60 NON-DISPLACING. Seat = Fable 5 until Nicholas declares settled (D-0134).
...
- **i59 (D-0154): the ROOT front-door migration SHIPPED** -- START_HERE -> boot KERNEL + NEW `COLD_BOOT_CARD.md` (generator-independent survivor) + NEW `ops/frontdoor/` uniform rebuild interface (registry + driver + frontdoor-gate) + the migration gate; 3-adversary red-teamed (architecture HELD, 13 gaps folded: rebuild-once->Mode-B, pinned survivor slot, frozen-redirect active-work, K_max doc bound, fail-closed-by-DETECTION close); DOC_PROTOCOL s2 front-door amendment; driver proven green (root-pcb 0-stale). Doc/ops close, no module code.

Runtime paths: plans `.../30-orchestrate-fanout/runtime/plans/<plan_id>/` * artifacts `.../runtime/artifacts/<id>/` * leases `.../29-resource-lease/runtime/leases/`. Waves + ad-hoc commits share one counter; **the next wave is iteration 60.**

## 4. Current frontier -- i59 CLOSED (D-0154): the ROOT migrated; NEXT = i60 first class migration; AUDIT review_due i60; SP3/M-03 open

**i59 CLOSED (D-0154).** The ROOT front-door migrated (D-0152/D-0153 s8 rule 1). `START_HERE.md` is now a stable ~1 KB hand-maintained boot **KERNEL** (routes Mode A packet / rebuild-once / Mode B card; holds NO changing state). NEW `core-docs/COLD_BOOT_CARD.md` = the generator-INDEPENDENT recovery survivor (`{kernel + card}` is the closed recovery set; the kernel alone is insufficient -- CB1): near-frozen fixed-slot (as_of, identity, SoT rule, 7 pinned never-spill constraints, count-asserted 23-doc list N<=K_max=30, frozen-redirect active-work, last-good SHA, generator-free read order). NEW `ops/frontdoor/` = the **uniform rebuild interface**: `registry.json` (one class `root-pcb`) + `Rebuild-FrontDoors.ps1` (the ONE O(1) verb the kernel names; non-vacuity floor) + `frontdoor-gate.py` (fail-closed close-step) + the **migration gate** (a class migrates only after registering a conformant {rebuild,verify} entry that rebuilds green). Design-first -> 3-adversary in-session red-team (D-0119): the `{kernel+card+uniform-rebuild}` architecture HELD; 13 gaps folded -- the FALSE 'atomic two-commit close' replaced by fail-closed-BY-DETECTION (Mode A refuses a packet whose epoch is behind the card -> Mode B; the card commits generator-independently in Phase 1, the N7 packet is the Phase-2 epoch anchor). Driver proven GREEN on-box (root-pcb 0-stale). **NEXT = i60 = the first CLASS migration** (CURRENT_STATE interior / handoff ledger / catalogs / backlogs / gotchas / tests / research), each design-first -> red-team-gated + passing the `frontdoor-gate.py` migration gate, carrying the s8 build requirements; the renderer slim-ladder is removed during migration. AUDIT review_due i60 NON-DISPLACING. SP3/M-03 OPEN; P0-1 activation FROZEN; SEALED_CHECK_47 retained; #40 beam-width / PB-2 / w08 deferred (D-0134).

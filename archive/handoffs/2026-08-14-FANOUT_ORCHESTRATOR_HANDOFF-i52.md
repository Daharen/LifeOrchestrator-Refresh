...
- **i51: the fresh legacy-vs-PCB gate RAN under the D-0140/N4 bars and returned NO-GO (D-0142). HEAD = the i51 docs-close commit. The LEGACY BOOTSTRAP (this doc) STAYS DEFAULT.** Quality floor (b) breached -- B under A 3.0-vs-4.0 on constraint adherence (both adjudicators; deductions = packet canon gaps, I51_RESULTS F3) -- and TOTAL missed both accountings (2.74x/1.12x A vs 0.7x; **BOOT PASS** 16,943 B; VOID clean; A/B pin held). Full record: `modules/44-project-map/eval/results/I51_RESULTS.md`. **i52 = the fix wave (Nicholas directive): N5 doc-section/`card:` granularity + N6 OPERATIONS canon extension (one #44 lane) + N7 close-time re-fold + N8 re-run re-freeze (ratification) -> only then a fresh gate.** The D-0140 bars stand. **Seat = Fable 5 until Nicholas declares settled (D-0134).**
...
- **i40-i42 (D-0110..D-0118):** mandate-02 licensed; #43 **0.4.0 -> 0.6.0** exact-closure arc (rounds 3-5) + #37 0.8.1 + **M2-A the doc-hygiene commit gate SHIPPED**; **round-5 PASS -> s7 RATIFIED (D-0118; the P0-1 arc closes)**; MODEL-TIERING adopted (D-0114).
- **i43-i45 (D-0120..D-0129):** NO-code live-GUI confirms + the expert-forensic finding -> **LRAP** (P9; AUDIT_PIPELINE promoted); i45 `fo-45-b17a531e` NEW widgets/08 (87/0/0; leveled accept) + the poser arc.
...
- **i48 (D-0135):** the CD-path wave `fo-48-3d3a4e1b` (1 coding lane, Opus) -- #44 **0.2.0**: CD-1 OPERATIONS boot canon + CD-3 query surface -- **ORCHESTRATOR-RECOVERED ship `c451890`** (no pwsh in the worker; independently verified pre-ship) + **the harvest sunset-fix `4055dc1`** (inline; 93->98) + fold (overlay i48) -> **VALIDATE 0**; render + `-Live Folded` green; **re-check RUN + FAILED same-session (D-0136): void x2, B 1.24x A; legacy stays default**.
...
## 4. Current frontier -- NEXT = i52 (the D-0142 fix wave: N5+N6 #44 lane, N7 close-currency, N8 ratification)

**i51 returned NO-GO (D-0142);** record + mechanisms: `modules/44-project-map/eval/results/I51_RESULTS.md` (F1 task-class -- prose-governing docs not query-granular, so B paid the boot layer ON TOP of A's prose corpus; F2 doc-only closes leave the shipped map stale -> its stale flags push whole-doc corroboration; F3 packet canon gaps = every scored B constraint deduction; F4 unfrozen listing/staged-not-read accounting). **i52 executes the fix wave (Nicholas directive):** ONE #44 coding lane (Opus 4.8 Extra) ships **N5** -- `section:` extended to doc entities + a `card:<id>` verb (accept: T1 cluster <= 8 KB query outputs, no whole-doc open) -- and **N6** -- OPERATIONS canon extension: D-0064 full-strength, K5 budgets+gate, mandate-02 sunset, rider state, red-team non-optional (accept: a fresh probe HITs K5/K6/K9/K10 unsoftened). The orchestrator lands **N7** -- close-time #44 re-fold on EVERY close incl. doc-only (accept: 0 stale on boot_read at HEAD) -- and stages **N8** for NICHOLAS RATIFICATION (two-class task pool: map-native AND prose-governing; pre-frozen listing/staged-not-read/tool-slice conventions; C-tier pin refresh). **A fresh gate stages only after N5-N7 land + N8 is ratified.** i48 + i51 packs = design input for the #40 beam-width wave.

**Deferred menu (D-0134, deferred NOT dropped):** (b) AUDIT next_increment (front step / ride-along / OUTPUT side; design-first -> red-team; **review_due i54, D-0137**; the w08 window-close defect rides the next w08 touch) · (c) KNOWLEDGE SURFACE RE-LAYER (D-0141/PB-7, subsumes PB-6): cumulative registries -> bounded-hot + indexed-cold on the PCB + memory engines · (d) #40 beam-WIDTH · (e) PB-2 (trigger-gated). **P0-1 ACTIVATION stays FROZEN. SEALED_CHECK_47 is NOT evaluated before i54.**
...
## 11. Box state at handoff (2026-08-14, D-0142 -- i51 gate NO-GO)

**i51: NO code wave -- the fresh migration gate RAN + returned NO-GO (D-0142); HEAD = the i51 docs-close commit.** No worker plan; eval sessions Nicholas-dispatched, eval-folder grant only; frozen tree untouched (0 x3). Landed: `eval/results/` I51_RESULTS.md + packs + verdicts + dispatches + materials-i51/ + MANIFEST-i51 (byte-verbatim); D-0142 + index row; CURRENT_STATE + this doc refreshed (incl. the D-0134 staleness fixes); outgoing snapshot archived. No held res.lease (durable `gpu-*` siblings persist by design); heartbeat `degraded:false`; 0 UNMANAGED orphans. Doc-gate PASS. Slots EMPTY. Eval folder + quarantine + i48/i51 worktrees on disk pending Nicholas disposition. **Open Nicholas items: (1) dispatch the i52 N5+N6 #44 lane; (2) ratify N8 when staged.** The w08 window-close defect stays routed to the next w08 touch (D-0134).

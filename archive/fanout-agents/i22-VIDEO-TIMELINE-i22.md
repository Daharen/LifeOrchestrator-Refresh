...
- **State:** done. **Commit `e8583d1`** (verified native git; exactly the 22 new modules/34 files, 2665
  insertions) + the ORCHESTRATOR FOLD **0.1.1 `bad9e27`** (9 files: score_unit honored verbatim;
  scene_index >= -1; float mode fail-closed at > 1; NEW recon suite + 4 fixtures embedding real #33 bytes;
  main-suite generator-version assertion updated).
- **Gates:** 0.1.0: 138/138 cloud + 138/138 `-Live` + dev.ship green + 7 cross-env hashes equal. 0.1.1: main
  ALL PASSED + recon 20/20 (cloud AND `-Live`; recon hashes equal cross-env) + dev.ship green (sha 9/9, AST
  3/3) + the cross-module smoke green end-to-end (case A evidence sane; case B scene_index -1 accepted).
- **Why 0.1.1 exists:** the isolation rule worked as designed -- both workers recorded their divergent digest
  readings in SCHEMA_NOTES.md, and the orchestrator's smoke (real #33 stable output into #34) surfaced exactly
  those two divergences; neither worker's own green gate could see them. Standing rule adopted (D-0077).
- **Follow-ons:** folded into MODULE_ROADMAP #34 (D-0077).

...
retrieval.eval #37 0.2.0->0.3.0 SHIPPED `99bb627` -- NEW lib/selpol_rrf_v1.py (s4/P1-1: hard-filter->temporal->authority->RRF-over-channel-ranks->occurrence-preserving-dedup->budget; additive output); rerank() now a thin wrapper (byte-identical A/B); eval-0.3 per-stage + packet_disposition. 27 library-unit + benchmark + cross-env -Live green. FOLD: D-0077 pair-1 -- the canonical selpol keeps raw-fused_score primary (deferring the rank-RRF-primary P1-2 switch), so it diverges from #40's rank-RRF reference -> i31 settle pins the §4 scoring.

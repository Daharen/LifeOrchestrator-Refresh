<#
    mock-invoke-skill.ps1 - a stand-in for modules/01-skill-bootstrap/Invoke-Skill.ps1 used ONLY by the
    Verification Console cloud pre-ship gate (a real Module run may be GPU-bound / Windows-only and cannot
    run on the Linux cloud box). It accepts the same -SkillDir / -InputsJson / -ArtifactRoot / -PwshPath the
    console passes and emits a canned but shape-accurate lifeorch.skill.invocation_report/0.1 (nesting a
    lifeorch.skill.result/0.1 envelope) to stdout, so the console core's spawn -> parse -> format path is
    exercised for real. (Identical to the Widget 02 mock - the run path is the same Module 1 wrapper contract.)
...
if ($inp -match 'NOISY') { Write-Output 'llama-server: loading model ... done' }

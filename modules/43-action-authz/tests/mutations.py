"""
mutations.py -- the mandatory SEEDED-MUTATION kill harness (contract s8.6, M-A01..M-E37).
...
from action_authz.boundary import (MockExecutor, evaluate_completion,
                                    capture_target_handles, apply_effects_through_handles,
                                    handle_consumption_proof)
from . import harness
...
def sec_e37(m):
    ...
    # captured one-shot TargetHandle, never a blind copy of the supplied effect tagged with the RAW
    # captured digest. Property (holds on the reference): the applicator produces a non-empty ledger where
    # every atom carries its bound target's canonical_target_id AND the one-shot CONSUMPTION proof (NOT the
    # raw captured digest), AND every captured handle it used is observably consumed. M-E37 blind-copies +
    # tags with the RAW digest WITHOUT consuming -> handles stay unconsumed (and the tag is the raw digest,
    # not the consumption proof) -> kill.
    targets = [{"target_kind": "fs.file", "canonical_target_id": "/u/data/projA/one.txt",
                "namespace": "projA", "resolution_kind": "existing", "resolution_proof_digest": "b" * 64}]
    effects = [{"effect_class": "fs.write", "target_index": 0, "quantity": 1, "unit": "bytes",
                "effect_risk_class": 2, "externality": "local", "reversibility": "compensatable"}]
    handles = capture_target_handles(targets)
    applied = apply_effects_through_handles(effects, handles, m)
    all_consumed = len(handles) > 0 and all(h.consumed for h in handles.values())
    expected = handle_consumption_proof("/u/data/projA/one.txt", "b" * 64)
    ledger_from_handles = (applied is not None and applied != []
                           and all(a.get("applied_via_handle") == expected
                                   and a.get("canonical_target_id") == "/u/data/projA/one.txt"
                                   for a in applied))
    return bool(ledger_from_handles and all_consumed)


def sec_e38(m):
    # i41 round-4 Finding 2: the round-4 SUCCESSOR mutant (the reviewer's stronger defect). The effect
    # ledger MUST be the handle-bound applicator RESULT -- each atom carries the CONSUMED handle's
    # canonical_target_id AND its one-shot CONSUMPTION proof. M-E38 consumes the handle (so it is observably
    # consumed) but DISCARDS the applicator result and blind-copies authorized_effect_set, reading the RAW
    # captured digest off the handle -> the atoms lack the consumption proof + canonical_target_id -> kill.
    st, d = _happy_permit(m)
    if d.outcome != "PERMIT":
        return False
    r = MockExecutor(st).execute(d.permit_ref, mutations=m)
    ...
    prov_ok = (r.accepted and r.state_diff != [] and all(
        a.get("canonical_target_id") == caps.get(a.get("target_index", 0), (None, None))[0]
        and a.get("applied_via_handle") == caps.get(a.get("target_index", 0), (None, None))[1]
        for a in r.state_diff))
    return bool(prov_ok)
...
REGISTRY = [
    ...
    ("M-E37", "effect", sec_e37, None),  # i41 round-3 Finding 2: blind authorized_effect_set copy+tag
    ("M-E38", "effect", sec_e38, None),  # i41 round-4 Finding 2: consume-but-discard-result + blind copy
]

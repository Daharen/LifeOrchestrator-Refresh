...
def sec_e37(m):
    # i41 round-3 Finding 2: the effect ledger is GENERATED by CONSUMING the captured one-shot
    # TargetHandle, never a blind copy of authorized_effect_set tagged with a digest. Property (holds on
    # the reference): the applicator produces a non-empty ledger AND every captured handle it used is
    # observably consumed. M-E37 blind-copies + tags WITHOUT consuming -> handles stay unconsumed -> kill.
    ...
    ledger_from_handles = (applied is not None and applied != []
                           and all(a.get("applied_via_handle") == "b" * 64 for a in applied))

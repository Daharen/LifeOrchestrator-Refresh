...
def _bd_handle_consumed():
    """i41 round-3 Finding 2: the effect ledger is GENERATED by CONSUMING the captured one-shot target
    HANDLE, not a blind copy of authorized_effect_set tagged with a digest. On a benign execute every
    applied atom carries applied_via_handle == its bound target's captured resolution_proof_digest AND the
    handle for every referenced target is OBSERVABLY CONSUMED (r.consumed_handles). A blind copy would
    consume no handle -- exactly the M-E37 mutant the suite now kills."""
    st, d = harness.run_happy()
    r = MockExecutor(st).execute(d.permit_ref)
    caps = {i: t["resolution_proof_digest"] for i, t in enumerate(d.permit["resolved_target_set"])}
    tags_ok = all(e.get("applied_via_handle") == caps.get(e.get("target_index", 0)) for e in r.state_diff)
    referenced = sorted({e.get("target_index", 0) for e in r.state_diff})
    consumed_ok = (r.consumed_handles == referenced and referenced != [])
    ok = r.accepted and r.state_diff != [] and tags_ok and consumed_ok
    return {"status": "pass" if ok else "fail",
            "baseline_expected": "each applied atom's applied_via_handle == captured handle AND every "
                                 "referenced target handle is observably consumed (r.consumed_handles)",
            "observed": "accepted=%s diff=%d consumed_handles=%s handles=%s"
                        % (r.accepted, len(r.state_diff), r.consumed_handles,

...
from action_authz.boundary import MockExecutor, handle_consumption_proof
...
def _bd_handle_consumed():
    """i41 round-3 Finding 2 (+ round-4 provenance): the effect ledger is GENERATED by CONSUMING the
    captured one-shot target HANDLE, not a blind copy of authorized_effect_set tagged with a digest. On a
    benign execute every applied atom carries its bound target's canonical_target_id AND its one-shot
    CONSUMPTION proof (handle_consumption_proof over the captured digest, NOT the raw captured digest), AND
    the handle for every referenced target is OBSERVABLY CONSUMED (r.consumed_handles). A blind copy would
    consume no handle -- exactly the M-E37 mutant the suite kills."""
    st, d = harness.run_happy()
    r = MockExecutor(st).execute(d.permit_ref)
    caps = {i: (t["canonical_target_id"],
                handle_consumption_proof(t["canonical_target_id"], t["resolution_proof_digest"]))
            for i, t in enumerate(d.permit["resolved_target_set"])}
    prov_ok = all(e.get("canonical_target_id") == caps.get(e.get("target_index", 0), (None, None))[0]
                  and e.get("applied_via_handle") == caps.get(e.get("target_index", 0), (None, None))[1]
                  for e in r.state_diff)
    referenced = sorted({e.get("target_index", 0) for e in r.state_diff})
    consumed_ok = (r.consumed_handles == referenced and referenced != [])
    ok = r.accepted and r.state_diff != [] and prov_ok and consumed_ok
    return {"status": "pass" if ok else "fail",
            "baseline_expected": "each applied atom carries the consumed handle's canonical_target_id + "
                                 "one-shot consumption proof AND every referenced handle is observably consumed",
            "observed": "accepted=%s diff=%d consumed_handles=%s prov_ok=%s"
                        % (r.accepted, len(r.state_diff), r.consumed_handles, prov_ok),
            "expected_fault_difference": "a blind authorized_effect_set copy+tag consumes NO handle "
                                         "(consumed_handles==[]) -- the killed M-E37 mutant",
            "observed_fault": "n/a"}


def _bd_ledger_provenance():
    """i41 round-4 Finding 2: the effect ledger ORIGINATES at the handle-bound applicator RESULT (each atom
    carries the CONSUMED handle's canonical_target_id + one-shot consumption proof; authorized_effect_set is
    ONLY the authorization bound), and the round-4 SUCCESSOR mutant M-E38 -- consume the handle but DISCARD
    the applicator result and blind-copy authorized_effect_set with the raw captured digest -- is DECISIVELY
    KILLED (its atoms lack the consumption proof + canonical_target_id)."""
    genuine = MUT.sec_e38(frozenset())               # reference: ledger provenance holds
    still_holds_under_mutant = MUT.sec_e38(frozenset(["M-E38"]))   # mutant: provenance broken
    ok = (genuine is True) and (still_holds_under_mutant is False)
    return {"status": "pass" if ok else "fail",
            "baseline_expected": "ledger atoms are the handle-bound applicator RESULT (canonical_target_id + "
                                 "one-shot consumption proof); authorized_effect_set is only the bound",
            "observed": "genuine_provenance=%s M-E38_killed=%s" % (genuine, not still_holds_under_mutant),
            "expected_fault_difference": "M-E38 consumes the handle but blind-copies authorized_effect_set "
                                         "(raw digest, no consumption proof) -> ledger provenance broken",
            "observed_fault": "n/a"}
...
def run(check):
    ...
    add("Boundary-D3:D4_handle_consumed", "boundary-D", "benign_execute", "captured-handle-consumption",
        ...
        _bd_handle_consumed())
    # i41 round-4 Finding 2: the ledger ORIGINATES at the handle-bound applicator RESULT; the round-4
    # successor mutant (consume the handle but discard the result + blind-copy authorized_effect_set) dies.
    add("Boundary-D3:D4_ledger_provenance", "boundary-D", "benign_vs_M-E38", "M-E38",
        "effect ledger provenance (handle-bound applicator result vs blind authorized_effect_set copy)",
        "the ledger is the handle-bound applicator RESULT (consumption proof + canonical_target_id); "
        "authorized_effect_set is only the bound; the consume-but-discard successor mutant is killed",
        _bd_ledger_provenance())
    ...
    # ---- every mutation M-A01..M-E37 (executed differential) ---------------------------------

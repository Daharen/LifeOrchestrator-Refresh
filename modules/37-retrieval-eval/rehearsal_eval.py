#!/usr/bin/env python3
# rehearsal_eval.py -- the i35 Tier-1 ACCEPTANCE-GATE REHEARSAL harness for retrieval.eval (Life Orchestrator
# module 37 `retrieval.eval` 0.7.0, contract v0.7; i35 Lane B REHEARSAL-HARNESS, plan fo-35-0a5bf334,
...
#
#   NAVIGATION COST is measured via #36's own `shortlist`/`descend` (shipped in 0.5.0), because #40's compile CLI
#   is FLAT-only today (its shortlist-and-descend plan needs an in-process live hierarchy_port; Lane A wires the
#   CLI). If a metric genuinely needs a #36/#40 output field that is ABSENT, the harness records a
#   FOLD_RECONCILIATION flag and refuses `tier1_accepted` (it never silently passes).
#
...
# ----------------------------------------------------------------- report
def build_report(benchmark, corpus_root, fixtures_dir, config, scales, fanout, adapter_cfg):
    module_dir = HERE
...
def run_request(req):
    ...
    report = build_report(benchmark, corpus_root, fixtures_dir, config, scales, fanout, adapter_cfg)

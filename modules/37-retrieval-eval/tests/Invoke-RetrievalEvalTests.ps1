...
# VERSION TRUTH (i40 PB-5 / D-0108): skill.json is the single declared version source. The harness pins the
# SHAPE here (well-formed semver), never a literal value, and below PERMANENTLY asserts that every envelope
# the skill emits carries EXACTLY the manifest's version -- so a future bump that misses any surface FAILS
# loudly (cloud + -Live) instead of sitting undetected (the i36..i39 0.7.0-literal drift class).
$manifestVersion = [string]$manifest.version
$manifestContract = [string]$manifest.contract_version
Check 'manifest skill_id retrieval.eval' ($manifest.skill_id -eq 'retrieval.eval')
Check 'manifest version well-formed semver (single source of truth; no literal pin -- D-0108)' ($manifestVersion -match '^\d+\.\d+\.\d+$')
Check 'manifest contract_version well-formed (single source of truth)' ($manifestContract -match '^\d+\.\d+$')
...
if ($null -ne $env1) {
    ...
    Check 'VERSION-TRUTH baseline: envelope skill_version == manifest version (permanent drift assertion)' ($env1.skill_version -eq $manifestVersion)
    Check 'VERSION-TRUTH baseline: envelope contract_version == manifest contract_version' ($env1.contract_version -eq $manifestContract)
...
if ($null -ne $env9) {
    ...
    Check 'VERSION-TRUTH eval04: envelope skill_version == manifest version' ($env9.skill_version -eq $manifestVersion)
...
if ($null -ne $env10) {
    ...
    Check 'VERSION-TRUTH eval05: envelope skill_version == manifest version' ($env10.skill_version -eq $manifestVersion)
...
if ($null -ne $py) {
    ...
    if ($null -ne $henv) {
        ...
        Check 'VERSION-TRUTH hierarchy: envelope skill_version == manifest version' ($henv.skill_version -eq $manifestVersion)
...
if ($null -ne $py) {
    ...
    if ($null -ne $renv) {
        ...
        Check 'VERSION-TRUTH rehearsal: envelope skill_version == manifest version' ($renv.skill_version -eq $manifestVersion)
